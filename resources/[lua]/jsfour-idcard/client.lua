...
function DriveRageUI(infos, infos_values, dob, sid)
    local _infos = infos
    local _infos_values = infos_values
local mugshot, mugshotStr = ESX.Game.GetPedMugshot(GetPlayerPed(GetPlayerFromServerId(sid)))
...
Citizen.CreateThread(function()
    while true do
        ...
        RenderText("~bold~Permis de Conduire ", 1350 + 400, 805, 0, 0.35, 255, 255, 255, 255, 1)
        RenderText('Date de naissance* ', 1350 + 377, 835, 0, 0.23, 202,203,204, 255, 1)
        RenderText(dob, 1350 + 447, 835, 0, 0.25, 255, 255, 255, 255, 0)

        for i = 1, #_infos do
            RenderText(_infos[i], 1350 + 150, 840 + 30 * i, 0, 0.30, 202,203,204, 255, 0)
        end
        for i = 1, #_infos_values do
            RenderText(_infos_values[i], 1350 + 520, 840 + 30 * i, 0, 0.30, 255, 255, 255, 255, 2)
        end
...
AddEventHandler('Game:Client:UI:showDrive', function(infos, infos_values, dob, sid)
    ...
    DriveRageUI(infos, infos_values, dob, sid)
end)
...
TriggerServerEvent('Game:Personal:Server:drive')
    if distance ~= -1 and distance <= 3.0 then
        TriggerServerEvent('Game:Personal:Server:drive', GetPlayerServerId(player))                                
    else
