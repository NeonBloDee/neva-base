...
function deletePlate()
    local total = 0 
    MySQL.Async.fetchAll("SELECT info, id FROM sunny_trunk", {}, function(trunk)
        if trunk[1] then
            for k, v in pairs(trunk) do 
                ...
                if OwnerVeh[data.plate] ~= nil then
                    ...
                    total = total + 1
                    MySQL.Async.fetchAll("DELETE FROM sunny_trunk WHERE id = @id", {
                        ['@id'] = v.id
                    }, function(result)
...
AddEventHandler('onResourceStart', function(resourceName)
    ...
    MySQL.Async.fetchAll("SELECT * FROM sunny_trunk", {}, function(result)
        for k, v in pairs(result) do
...
RegisterServerCallback("lgdddd:getChestVehicle", function(source, cb, plate)   
...
function saveTrunk()
    ...
    for k, v in pairs(VehCoffre) do
        MySQL.Sync.execute("UPDATE sunny_trunk set info = @info, data = @data WHERE id = @id", {
            ["@id"] = v.id,
...
-----------------------------------------------------------------------------------------------------------------------------------------------------------
-- RegisterNetEvent("lgd:takeInTrunk")
-- AddEventHandler("lgd:takeInTrunk", function(id, plate, max)
--     local source = source
--     local xPlayer = GetPlayerFromId(source)

--     MySQL.Async.execute("UPDATE sunny_clothes SET identifier = @identifier, trunk = @trunk WHERE id = @id", {
...
RegisterNetEvent("lgd:actionClothTrunk")
AddEventHandler("lgd:actionClothTrunk", function(action, vehicle, data)
    local xPlayer = GetPlayerFromId(source)
    ...
    if action == "deposit" then
        ...
        MySQL.Async.execute("UPDATE sunny_clothes SET identifier = @identifier WHERE id = @id", {
...
CreateVehToBdd = function(plate, class)
    ...
    MySQL.Async.execute("INSERT INTO sunny_trunk (info, id) VALUES (@info, @id)", {
...
RegisterNetEvent("lgdddd:actionItem", function(plate, class, action, count, name)
    local xPlayer = GetPlayerFromId(source)
	...
	if check == false then 
        ...
        -- Vérification pour s'assurer que count est supérieur à 0
        count = tonumber(count)
        if count <= 0 then
            showNotification(xPlayer, Locales[Config.Language]['invalid_quantity'], 'error')
            return
        end
            ...
            if InfoItem.count >= tonumber(count) then
                if not VerifExistPlate then
                    ...
                    if CreateVeh then
                        ...
                        exports['core']:sendLog(("Dépot d'objet(s) dans un vehicule non joueur (%s - %s) Plaque (%s) (Coffre)"):format(xPlayer.name, xPlayer.UniqueID, plate), {
                            author = xPlayer.name,
                            fields = {
                                {title = 'Joueur', subtitle = xPlayer.name},
                                {title = 'ID Unique', subtitle = xPlayer.UniqueID},
                                {title = 'Identifier', subtitle = xPlayer.identifier},
                                {title = 'Item Déposé / Qauntité', subtitle = ('%s / %s'):format(InfoItem.label, count)},
                    ...
                    if VerifWeigt then
                        ...
                        RemoveItem(xPlayer, name, count)
                        exports['core']:sendLog(("Dépot d'objet(s) dans un vehicule (%s - %s) Plaque (%s) (Coffre)"):format(xPlayer.name, xPlayer.UniqueID, plate), {
                            author = xPlayer.name,
                            fields = {
                                {title = 'Joueur', subtitle = xPlayer.name},
                                {title = 'ID Unique', subtitle = xPlayer.UniqueID},
            ...
            if GetWeightPlayer(xPlayer, InfoItem.name, count) then
                if VehCoffre[plate].data["items"][InfoItem.name] ~= nil then
                    if VehCoffre[plate].data["items"][InfoItem.name].count >= tonumber(count) then
                        ...
                        exports['core']:sendLog(("Retrait d'objet dans un véhicule (%s - %s) Plaque (%s) (Coffre)"):format(xPlayer.name, xPlayer.UniqueID, plate), {
                            author = xPlayer.name,
                            fields = {
                                {title = 'Joueur', subtitle = xPlayer.name},
                                {title = 'ID Unique', subtitle = xPlayer.UniqueID},
                                {title = 'Identifier', subtitle = xPlayer.identifier},
                                {title = 'Item pris / Qauntité', subtitle = ('%s / %s'):format(InfoItem.label, count)},
                                {title = 'Plaque', subtitle = plate},
                                {title = "Discord", subtitle = ESX.GetIdentifierFromId(source, "discord:")},
                            },
...
RegisterNetEvent("lgdddd:actionsWeapon", function(action, vehicle, data)
    local xPlayer = GetPlayerFromId(source)
    local _itemName = name
	local check = exports['ArmePerm']:sv_isitemperm(_itemName)
	if check == false then 
                ...
                if not VerifExistPlate then
                    ...
                    if CreateVeh then
                        ...
                        if VerifWeigt then
                            ...
                            exports['core']:sendLog(("Dépôt d'une arme dans un véhicule non joueur (%s - %s) Plaque (%s) (Coffre)"):format(xPlayer.name, xPlayer.UniqueID, vehicle.plate), {
                                author = xPlayer.name,
                                fields = {
                                    {title = 'Joueur', subtitle = xPlayer.name},
                                    {title = 'ID Unique', subtitle = xPlayer.UniqueID},
                                    {title = 'Identifier', subtitle = xPlayer.identifier},
                                    {title = 'Arme', subtitle = data.label},
                                    {title = 'Plaque', subtitle = vehicle.plate},
                                    {title = "Discord", subtitle = ESX.GetIdentifierFromId(source, "discord:")},
                                },
                    ...
                    if VerifWeigt then
                        ...
                        removeWeapon(xPlayer, data.name)
                        exports['core']:sendLog(("Dépôt d'une arme dans un véhicule (%s - %s) Plaque (%s) (Coffre)"):format(xPlayer.name, xPlayer.UniqueID, vehicle.plate), {
                            author = xPlayer.name,
                            fields = {
                                {title = 'Joueur', subtitle = xPlayer.name},
                                {title = 'ID Unique', subtitle = xPlayer.UniqueID},
        ...
        elseif action == "remove" then
            -- if GetWeightPlayer(xPlayer, name, count) then
                ...
                if VehCoffre[vehicle.plate].data["weapons"][data.name] ~= nil then
                    ...
                    exports['core']:sendLog(("Retrait d'une arme dans un véhicule (%s - %s) Plaque (%s) (Coffre)"):format(xPlayer.name, xPlayer.UniqueID, vehicle.plate), {
                        author = xPlayer.name,
                        fields = {
                            {title = 'Joueur', subtitle = xPlayer.name},
                            {title = 'ID Unique', subtitle = xPlayer.UniqueID},
...
RegisterNetEvent("lgdddd:actionsAccount", function(action, vehicle, data)
    local xPlayer = GetPlayerFromId(source)
    local VerifExistPlate = VehCoffre[vehicle.plate] ~= nil and true or false
        ...
        if getAccount(xPlayer, data.type) >= tonumber(data.count) then
            if not VerifExistPlate then
                ...
                if CreateVeh then
                    ...
                    exports['core']:sendLog(('Depot argent Dans un vehicule non joueur (%s - %s) Plaque (%s) (Coffre)'):format(xPlayer.name, xPlayer.UniqueID, vehicle.plate), {
                    author = xPlayer.name,
                    fields = {
                        {title = 'Joueur', subtitle = xPlayer.name},
                        {title = 'ID Unique', subtitle = xPlayer.UniqueID},
                        {title = 'Identifier', subtitle = xPlayer.identifier},
                        {title = 'Argent', subtitle = ('%s %s'):format(data.count, data.type)},
                        {title = 'Plaque', subtitle = vehicle.plate},
                        {title = "Discord", subtitle = ESX.GetIdentifierFromId(source, "discord:")},
                    },
                ...
                removeMoney(xPlayer, data.type, data.count)
                exports['core']:sendLog(('Depot argent Dans un vehicule (%s - %s) Plaque (%s) (Coffre)'):format(xPlayer.name, xPlayer.UniqueID, vehicle.plate), {
                    author = xPlayer.name,
                    fields = {
                        {title = 'Joueur', subtitle = xPlayer.name},
                        {title = 'ID Unique', subtitle = xPlayer.UniqueID},
    ...
    elseif action == "remove" then
        ...
        if VehCoffre[vehicle.plate].data["accounts"][data.type].count >= tonumber(data.count) then
            ...
            exports['core']:sendLog(("Retrait d'argent dans un vehicule (%s - %s) Plaque (%s) (Coffre)"):format(xPlayer.name, xPlayer.UniqueID, vehicle.plate), {
                author = xPlayer.name,
                fields = {
                    {title = 'Joueur', subtitle = xPlayer.name},
                    {title = 'ID Unique', subtitle = xPlayer.UniqueID},
