...
window.addEventListener("message", function (event) {
  ...
  if (event.data.action == "open:Inv") {
    ...
    let maxBarValue =
      (maxWeightBarValue * event.data.weight) / event.data.maxWeight;
    weightBar.css("width", maxBarValue + "vw");
    weightText.text(event.data.text);
  } else if (event.data.action == "setItems") {
    inventorySetup(
      event.data.itemList,
      event.data.fastItems,
      event.data.crMenu,
      event.data.itemTrunk
    );
    ...
    $.post("http://inv/cancel", JSON.stringify({}));
    ...
    style.innerHTML =
      ".idcard-container:after { border-bottom: 0.154vw solid " + color + "; }";
    ...
    let maxBarValue =
      (maxWeightBarValue * event.data.weightTrunk) / event.data.maxWeightTrunk; //Math.floor(fillPercentage);
...
function inventorySetup(items, fastItems, crMenu, itemTrunk) {
  ...
  if (crMenu == "item") {
    ...
    var i;
    image =
      "https://cdn.discordapp.com/attachments/1178796323986747392/1226130000256700446/ak-47_1.png?ex=6623a58c&is=6611308c&hm=fdfff0b7092317375dffb976743558266d8d5e44b7c33c1e323193e3da7b5f8c&";
...
$(document).ready(function () {
  $(document).keydown(function (event) {
    if (event.which === 9) {
      // 9 est le code de la touche TAB
      ...
      if (inventoryVisible) {
        ...
        $(".form-inv").fadeOut(100);
        inventoryVisible = false;
        $.post("http://inv/close", JSON.stringify());
  ...
  $("#button\\.valider").click(function () {
    $.post(
      "http://inv/send",
      JSON.stringify({ text: document.getElementById("textArea").value.trim() })
    );
    document.getElementById("textArea").value = "";
    $("#keyboard-input").hide();
  });
...
function NotifyItem(message, icon, number) {
  ...
  if (icon == undefined) {
    icon = "https://i.postimg.cc/1zxxjCL4/default-4d471008.png";
  }
...
$(document).mousedown(function (event) {
  ...
  itemInventory = $(event.target).data("inv");
