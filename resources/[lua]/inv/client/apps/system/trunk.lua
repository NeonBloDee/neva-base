...
function getDataVehicle(plate)
    ...
    InfoVehicle = {}
    ClothesVehicle = {}
    TriggerServerCallback("lgdddd:getChestVehicle", function(data, clothes)
...
function setTrunkInventoryData(plate, model, class)
    local maxWeightTrunk

    -- Convertir le hash du modèle en nom de modèle
    local modelName = GetDisplayNameFromVehicleModel(model):lower()
    print("Nom du modèle après conversion :", modelName)

    -- Vérifie si le véhicule a un poids spécifique défini
    if Config.WeightVehicle.SpecificVehicles[modelName] then
        maxWeightTrunk = Config.WeightVehicle.SpecificVehicles[modelName]
        -- print("Poids spécifique trouvé pour", modelName, ":", maxWeightTrunk)
    else
        maxWeightTrunk = Config.WeightVehicle[class]
        -- print("Aucun poids spécifique trouvé, utilisation du poids de classe :", maxWeightTrunk)
    end

    TriggerServerCallback("lgdddd:getChestVehicle", function(data)
        if data == 'nil' then
            textTrunk = tonumber(0).." / "..tonumber(maxWeightTrunk).. Locales[Config.Language]['weight_unity']
            SendNUIMessage(
                {
                    action = "trunk:WeightBarText",
                    weightTrunk = tonumber(0),
                    maxWeightTrunk = tonumber(maxWeightTrunk),
                    textTrunk = textTrunk,
...
RegisterNUICallback("PutIntoTrunk", function(data, cb)
    ...
    if data.item.type == 'item_vetement' then
        ...
        TriggerServerEvent("lgd:actionClothTrunk", 'deposit', dataVehicleIn, dataClothe)
        Wait(150)
        RefreshTrunk()
            ...
            KeyboardUtils.use(Locales[Config.Language]['quantite'], function(result) 
                if result ~= nil then
                    if tonumber(result) then
                        TriggerServerEvent("lgdddd:actionItem", dataVehicleIn.plate, dataVehicleIn.class, "deposit", tonumber(result), data.item.name)
                        Wait(150)
                        RefreshTrunk()
                    end
                end
        ...
        KeyboardUtils.use(Locales[Config.Language]['quantite'], function(result) 
            if result ~= nil then
                if tonumber(result) then
                    ...
                    TriggerServerEvent("lgdddd:actionsAccount", "deposit", dataVehicleIn, dataMoney)
                    Wait(150)
                    RefreshTrunk()
                end
            end
...
RegisterNUICallback("TakeFromTrunk", function(data, cb)
    ...
    if data.item.type == 'item_vetement' then
        ...
        TriggerServerEvent("lgd:actionClothTrunk", 'remove', dataVehicleIn, dataClothe)
        Wait(150)
        RefreshTrunk()
    elseif data.item.type == 'item_standard' then
        KeyboardUtils.use(Locales[Config.Language]['quantite'],function(result) 
            if result ~= nil then
                if tonumber(result) then
                    TriggerServerEvent("lgdddd:actionItem", dataVehicleIn.plate, dataVehicleIn.class, "remove", tonumber(result), data.item.name)
                    Wait(150)
                    RefreshTrunk()
                end
            end
    ...
    elseif data.item.type == 'item_weapon' then
        ...
        TriggerServerEvent("lgdddd:actionsWeapon", "remove", dataVehicleIn, dataWeapon) 
        Wait(150)
        RefreshTrunk()
        ...
        KeyboardUtils.use(Locales[Config.Language]['quantite'],function(result) 
            if result ~= nil then
                if tonumber(result) then
                    ...
                    TriggerServerEvent("lgdddd:actionsAccount", "remove", dataVehicleIn, dataMoney)
                    Wait(150)
                    RefreshTrunk()
                end
            end
        ...
        if not IsPedSittingInAnyVehicle(PlayerPedId()) then 
            ...
            if distance ~= -1 and distance <= 3.0 then
                ...
                if locked == 1 then
                    ...
                    ESX.ShowNotification(Locales[Config.Language]['trunk_vehicle_close'], 'error') 
                end
            else
                ESX.ShowNotification(Locales[Config.Language]['trunk_vehicle_distance'], 'error')
            end
        else
            ESX.ShowNotification(Locales[Config.Language]['trunk_vehicle_no_in'], 'error')
        end
