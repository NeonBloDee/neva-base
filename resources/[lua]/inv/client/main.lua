...
function removePedInv(bool)
    Inv.noPedInv = GetFieldValueFromName('inv').pedscreen
    if Inv.noPedInv == false or  Inv.noPedInv == nil then
        Inv.noPedInv = true
        SetFieldValueFromNameEncode('inv', {pedscreen = bool})
    else
        if Inv.noPedInv == true then
            Inv.noPedInv = false
            SetFieldValueFromNameEncode('inv', {pedscreen = bool})
...
Inv.FastWeapons = GetFieldValueFromName('inv').name and GetFieldValueFromName('inv').name or {}
                ...
                if Config.ActiveIdCard then
                    if dataInv.idCard ~= nil then
                        for k, v in pairs(dataInv.idCard) do
                            if v.type and Config.IdCardName[v.type] then
                                local nameCard = Config.IdCardName[v.type].name
                                local idcardData = {
                                    label = nameCard,
                                    count = 0.5,
                                    limit = 0,
                                    type = "item_idcard",
                                    name = v.type,
                                    image = Config.Pictures[v.type] or 'default_image', -- Utilisez une image par défaut si aucune n'est trouvée
                                    value = v.information,
                                    id = v.id,
                                    usable = true,
                                    rare = true,
                                ...
                                print("Erreur: type ou Config.IdCardName[" .. tostring(v.type) .. "] est nil")
                            end
                        end
                    else
                        print("Erreur: dataInv.idCard est nil")
                    end
                end
        ...
        if data.item.type == "item_standard" then 
            if Config.Framework == "qb" then
                ...
                if Config.IdCardName[data.item.name] then 
                    ...
                    print("open:MenuIdCard",json.encode(value),data.item.name)
                    SendNUIMessage({
