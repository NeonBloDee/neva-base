$(document).ready(function() {
    ...
    btn.click(function() {
        $.post('http://dj/togglePlaystate', JSON.stringify({}));
    ...
    directPlay.click(function() {
        $.post('http://dj/playNewSong', JSON.stringify({
            link: $('#linkInput').val()
        }));
        return;
    });
    ...
    buttonTwo.click(function() {
        $.post('http://dj/rewind', JSON.stringify({}));
        $('#timeLineInside').width($('#timeLineInside').width()-(perc*10))
        return;
    });

    buttonThree.click(function() {
        $.post('http://dj/forward', JSON.stringify({}));
        $('#timeLineInside').width($('#timeLineInside').width()+(perc*10))
        return;
    });

    buttonFour.click(function() {
        $.post('http://dj/down', JSON.stringify({}));
        return;
    });

    buttonFive.click(function() {
        $.post('http://dj/up', JSON.stringify({}));
        return;
    });
    ...
    AddPlaylist.click(function() {
        ...
        if ($('#CreatePlaylistName').val() != '') {
            ...
            $.post('http://dj/addPlayList', JSON.stringify({
    ...
    $(document).on('click', '.deleteSongs', function() {
        ...
        $.post('http://dj/deleteSong', JSON.stringify({
    ...
    $(document).on('click', '.deletePlaylist', function() {
        ...
        $.post('http://dj/deletePlaylist', JSON.stringify({
        ...
        if ($('#linkInput').val()) {
            ...
            $.getJSON('https://noembed.com/embed?url=', {format: 'json', url: $('#linkInput').val()}, function (data) {
                if (data.title) {
                    ...
                    $.post('http://dj/addSongToPlaylist', JSON.stringify({
                        id: i.data('id'),
                        link: $('#linkInput').val()
                    }));
                    ...
                    $.post('http://dj/noSongtitle', JSON.stringify({}));
                }
            });
    ...
    $(document).on('click', '.song', function() {
        ...
        $.post('http://dj/playSongFromPlaylist', JSON.stringify({
            id: $(this).data('id'),
            link: $(this).data('link'),
    ...
    function Close(event) {
        if (event.keyCode === 27) {
            $('body').hide()
            $.post('http://dj/close', JSON.stringify({}));
        }
    }
