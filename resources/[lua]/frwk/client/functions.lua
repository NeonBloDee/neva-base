...
function ESX.ShowNotification(message, notifyType, length)
	if GetResourceState('notif') == 'started' then
			exports.notif:Send(message, length, notifyType)
	end
end

function ESX.ShowAdvancedNotification(sender, subject, msg, textureDict, iconType)
	if GetResourceState('notif') == 'started' then
			exports.notif:SendAdvanced(msg, sender, subject, textureDict, iconType)
	end
end
...
-- 		TriggerEvent('sunny:inventory:notifi', msg)
-- 	else
...
-- 	exports['SunnyUI']:HelpNotification(data)

-- 	-- Configurer un timer pour enlever la notification après 2 secondes
-- 	Citizen.Wait(2000, function()
...
RegisterNetEvent('sunny:frammework:recieveItemsUsable', function(name)
...
RegisterNetEvent('sunny:viePlayers')
AddEventHandler('sunny:viePlayers', function(vie)
...
Citizen.CreateThread(function()
    while true do
        ...
        if not HealthLoaded or currentHealth ~= HealthCache then
            ...
            HealthLoaded = true
            TriggerServerEvent('sunny:SavePlayersData', currentHealth, currentArmor)
        end
