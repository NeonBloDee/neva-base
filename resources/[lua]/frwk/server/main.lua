...
function LoadUser(source, identifier)
	...
	table.insert(tasks, function(cb)
		MySQL.Async.fetchAll('SELECT UniqueID, playerName, permission_group, permission_level, accounts, job, job_grade, job2, job2_grade, firstname, lastname, sex, dateofbirth, inventory, loadout, position, rank, sunnycoins, totalCoins, boutiquereward FROM users WHERE identifier = @identifier', {
			...
			if result[1].sunnycoins then
				userData.sunnycoins = result[1].sunnycoins
			else
				userData.sunnycoins = 0
			end
...
AddEventHandler('esx:giveInventoryItem', function(target, type, itemName, itemCount)
	...
	if check == false then
		...
		sourcexPlayer.showNotification("Vous ne pouvez pas faire cela, cet item est permanent.")
	end
end)
...
-- 		if xItem2 then
-- 			TriggerClientEvent('sunny:framework:sortWeapon', source, itemName)
-- 		end
-- 	else
...
RegisterNetEvent('sunny:framework:savePlayers', function(source)
	ESX.SavePlayers()
end)

RegisterNetEvent('sunny:restartFrr', function()
	local source = source
