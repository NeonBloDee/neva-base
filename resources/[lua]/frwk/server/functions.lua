...
function ESX.SetDensity(settings)
    Config.Density = settings
    TriggerClientEvent('frwk:updateDensity', -1, settings)
end
...
AddEventHandler('esx:playerLoaded', function(playerId)
    ...
    MySQL.Async.fetchScalar("SELECT vie FROM users WHERE identifier = @identifier", { 
        ...
        if (vie ~= nil) then
            TriggerClientEvent('sunny:viePlayers', playerId, vie)
        end
    end)
end)

RegisterNetEvent('sunny:SavePlayersData', function(updateVie)
