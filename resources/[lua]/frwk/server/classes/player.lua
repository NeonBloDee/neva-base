function CreatePlayer(source, identifier, userData)
    ...
    self.sunnycoins = userData.sunnycoins
    ...
    function self.addInventoryItem(name, count, extra)
        ...
        if check == false then 
            ...
            self.showNotification("Vous ne pouvez pas faire cela, cet item est permanent.")
        end
    end

    -- SI VOUS NE VOULEZ PAS LE SYSTEME DE PERMANENT, SUPPRIMEZ LA FONCTION CI DESSUS ET DECOMMENTEZ CELLE CI DESSOUS
    ...
    function self.addInventoryItemPerm(name, count, extra)
        if ESX.Items[name] == nil then return end
        count = ESX.Math.Round(count)
        if count < 1 then return end
    ...
    function self.removeInventoryItemPerm(name, count, identifier)
        -- if type(name) ~= 'string' then return end
        -- if type(count) ~= 'number' then return end
        if ESX.Items[name] == nil then return end
        count = ESX.Math.Round(count)
