...
RegisterNetEvent('animmenu:openMenu', function()
    openMenu()
end)
...
RegisterNetEvent('animmenu:closeMenu', function()
    closeMenu()
end)
        ...
        elseif data.category == "syncedemotes" then
            ...
            if next(nearbyPlayers) ~= nil and next(nearbyPlayers) then
                ...
                for _, id in pairs(nearbyPlayers) do
                    exports['0r-textui']:create3DTextUIOnPlayers("animmenu-request-players-" .. id, {
                        id = id,
                        displayDist = 5.0,
                        interactDist = 1.3,
                        ...
                        theme = "green", -- or red
                        triggerData = {
                            triggerName = "animmenu:sendAnimRequest:client",
                ...
                Citizen.CreateThread(function()
                    while true do
                        ...
                        if IsControlPressed(0, 322) then
                            ...
                            for _, id in pairs(nearbyPlayers) do
                                exports['0r-textui']:delete3DTextUIOnPlayers("animmenu-request-players-" .. id)
                            end
                            break
                        end
                    end
...
RegisterNetEvent('animmenu:sendAnimRequest:client', function(data)
    ...
    TriggerServerEvent('animmenu:sendAnimRequest:server', data)
end)

local requestReceiveActive = false
RegisterNetEvent('animmenu:receiveAnimRequest:client', function(data)
    exports["0r-textui"]:displayTextUI(Lang:t("notifications.waiting_for_a_decision"), "ESC")
    requestReceiveActive = true
    exports['0r-textui']:create3DTextUIOnPlayers("animmenu-request-player-" .. data.target, {
        id = data.target,
        displayDist = 5.0,
        interactDist = 1.3,
        enableKeyClick = true, -- If true when you near it and click key it will trigger the event that you write inside triggerData
        ...
        theme = "green", -- or red
        triggerData = {
            triggerName = "animmenu:playAnimTogetherReceiver:client",
            args = {data = data}
        }
    })
    Citizen.CreateThread(function()
        while true do
            ...
            if IsControlPressed(0, 322) then
                ...
                exports["0r-textui"]:hideTextUI()
                exports['0r-textui']:delete3DTextUIOnPlayers("animmenu-request-player-" .. data.target)
                TriggerServerEvent('animmenu:requstCanelledNotif:server', data.target)
                break
            end
        end
    end)
...
RegisterNetEvent('animmenu:requstCanelledNotif:client', function()
    Config.Notify(Lang:t("notifications.request_cancelled"), 7500, "error")
end)

RegisterNetEvent('animmenu:playAnimTogetherReceiver:client', function(data)
    requestReceiveActive = false
    exports["0r-textui"]:hideTextUI()
    exports['0r-textui']:delete3DTextUIOnPlayers("animmenu-request-player-" .. data.data.target)
    local inVehicle = IsPedInAnyVehicle(PlayerPedId(), true)
    if inVehicle then return end
    TriggerServerEvent('animmenu:playAnimTogetherSender:server', data)
    ...
    --     syncOffsetFront = animOptions.SyncOffsetSide + 0.0
    -- end
...
RegisterNetEvent('animmenu:playAnimTogetherSender:client', function(data)
    local inVehicle = IsPedInAnyVehicle(PlayerPedId(), true)
    if inVehicle then return end
    local animOptions = data.data.data.AnimationOptions
    -- Coords
