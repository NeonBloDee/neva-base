        ...
        CreateThread(function()
            while code_menu do
                ...
                RageUI.IsVisible(menu_code, function()
                    ...
                    RageUI.Separator(string.format("%s↓↓~w~  Auto-école  %s↓↓~w~", SunnyConfigServ.GTACOLOR, SunnyConfigServ.GTACOLOR))
                    RageUI.Line()

                    if not code then
                        RageUI.Button('Effectuer le code de la route', nil, {RightLabel = ("~g~%s$~s~ →→"):format(SunnyConfigServ.ConfigAutoEcole.PrixCode)}, true, {
                            onSelected = function()
                                TriggerServerEvent('rx:BuyLicense', SunnyConfigServ.ConfigAutoEcole.PrixCode)
                            end
                        })
                    else
                        if not voiture then
                            RageUI.Button("Permis de voiture", nil, {RightLabel = ("~g~%s$~s~ →→"):format(SunnyConfigServ.ConfigAutoEcole.PrixPermisVoiture)}, true, {
                                onSelected = function()
                                    TriggerServerEvent('rx:BuyPermis', SunnyConfigServ.ConfigAutoEcole.PrixPermisVoiture, 'drive')
                                    FreezeEntityPosition(PlayerPedId(), false)
                                    code_menu = false
                                    RageUI.CloseAll()
                                end
                        ...
                        if not moto then
                            RageUI.Button("Permis de moto", nil, {RightLabel = ("~g~%s$~s~ →→"):format(SunnyConfigServ.ConfigAutoEcole.PrixPermisMoto)}, true, {
                                onSelected = function()
                                    TriggerServerEvent('rx:BuyPermis', SunnyConfigServ.ConfigAutoEcole.PrixPermisMoto, 'drive_bike')
                                    FreezeEntityPosition(PlayerPedId(), false)
                                    code_menu = false
                                    RageUI.CloseAll()
                                end
                        ...
                        if not camion then
                            RageUI.Button("Permis de camion", nil, {RightLabel = ("~g~%s$~s~ →→"):format(SunnyConfigServ.ConfigAutoEcole.PrixPermisCamion)}, true, {
                                onSelected = function()
                                    TriggerServerEvent('rx:BuyPermis', SunnyConfigServ.ConfigAutoEcole.PrixPermisCamion, 'drive_truck')
                                    FreezeEntityPosition(PlayerPedId(), false)
                                    code_menu = false
                                    RageUI.CloseAll()
                                end
                ...
                RageUI.IsVisible(menu_examen, function()
                    ...
                    RageUI.Button('Commencer le code de la route', nil, {RightLabel = ("→→")}, true, {
                        onSelected = function()
                            ...
                            timer_active = true
                            Notification(("[~y~Information~s~] Vous commencez votre code de la route, vous avez ~g~%s minutes~s~."):format(SunnyConfigServ.ConfigAutoEcole.timer/60))
                        end
                    }, menu_examen_1)
                end)
...
CreateThread(function()
    ...
    AddZones('AutoEcolePoint', {
        Position = SunnyConfigServ.ConfigAutoEcole.Pos,
        Dist = 10,
        Public = true,
        Job = nil,
        Job2 = nil,
