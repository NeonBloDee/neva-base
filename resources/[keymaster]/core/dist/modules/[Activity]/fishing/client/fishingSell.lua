CreateThread(function()
            ...
            RageUI.IsVisible(main, function()
                for k,v in pairs(SunnyConfigServ.Fishing.fishList.FishList) do
                    for i,p in pairs(ESX.GetPlayerData().inventory) do
                        for _, a in pairs(SunnyConfigServ.Fishing.fishList.FishList[k]) do
                            if tostring(p.name) == tostring(a.name) then
                                if p.count > 0 then
                                    RageUI.Button(('%s (%s)'):format(p.name, p.count), nil, {RightLabel = (''..SunnyConfigServ.GTACOLOR..'%s~s~ ~g~%s~s~ →'):format(zonesNames[k], a.price)}, true, {
                                        onSelected = function()
                                            KeyboardUtils.use('Quantité à vendre', function(data)
                                                ...
                                                TriggerServerEvent('SunnyServ:fishing:sell', a.name, a.label, a.price, data)
                                            end)
                                        end
                                    })
                                end
    ...
    _PEDS.addPed('fishing_sell', {
        ...
        position = SunnyConfigServ.Fishing.PosSell,
