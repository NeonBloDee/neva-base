CreateThread(function()
    ...
    for k,v in pairs(SunnyConfigServ.Fishing.fishList.FishList) do
        for i,p in pairs(SunnyConfigServ.Fishing.fishList.FishList[k]) do
    ...
    RegisterNetEvent('SunnyServ:fishing:rod:paid', function(name, price)
        local source = source
    ...
    ESX.RegisterServerCallback('SunnyServ:fishing:getInformations', function(source, cb)
        local source = source
    ...
    RegisterNetEvent('SunnyServ:fishing:permit:paid', function(type, price, type)
        local source = source
        local xPlayer = ESX.GetPlayerFromId(source)
        ...
        MySQL.Async.execute('UPDATE player_fishing SET permit = @permit WHERE UniqueID = @UniqueID', {
            ...
            TriggerClientEvent('SunnyServ:fishing:permit:startInstruction', source, type)
    ...
    RegisterNetEvent('SunnyServ:fishing:permit:finish', function()
        local source = source
        ...
        for k,v in pairs(SunnyConfigServ.Fishing.fishList.FishingRod) do
            ESX.RegisterUsableItem(v.name, function(source)
                ...
                if xPlayer.getInventoryItem(v.name).extra then
                    TriggerClientEvent('SunnyServ:fishing:zone:start', source, v.name, xPlayer.getInventoryItem(v.name).extra.fish)
                else
                    TriggerClientEvent('SunnyServ:fishing:zone:start', source, v.name, 0)
                end
            end)
        end
    end)

    RegisterNetEvent('SunnyServ:fishing:shop:lures:paid', function(price, name, label)
        local source = source
        local xPlayer = ESX.GetPlayerFromId(source)
