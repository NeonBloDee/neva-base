RegisterServerEvent('sDrugs:CheckAndActivateDrugsSell')
AddEventHandler('sDrugs:CheckAndActivateDrugsSell', function()
    local source = source 
    local xPlayer = ESX.GetPlayerFromId(source)
    local hasDrugs = false

    for _, itemData in pairs(SunnyDrugs.SaleItems) do
    ...
    if hasDrugs then
        TriggerClientEvent('sDrugs:StartDrugsSell', source)
    else
        TriggerClientEvent('esx:showNotification', source, "~r~Vous n'avez pas de drogues.")
    end
end)

RegisterServerEvent("sDrugs:SellDrugs")
AddEventHandler("sDrugs:SellDrugs", function(count)
    local xPlayer = ESX.GetPlayerFromId(source)
    ...
    TriggerClientEvent('sDrugs:DeactivateSell', source)
