CreateThread(function()
    ...
    CreateThread(function()
        Wait(1000)
        TriggerServerEvent('sunny:labs:fetchAll')
        TriggerServerEvent('sunny:labs:requireLabsInteriors', _LabsSystem.labsList)
    end)

    RegisterNetEvent('sunny:labs:recieve', function(labs)
                ...
                for k,v in pairs(_LabsSystem.list) do
                    ...
                    if dist < 2 then
                        ...
                        if IsControlJustPressed(0, 54) then
                            TriggerServerEvent('sunny:labs:enter', v.id)
                        end
                    end
    ...
    RegisterNetEvent('sunny:labs:enter', function(type, exitCoords, id)
                ...
                for k,v in pairs(_LabsSystem.list) do
                    if v.id == id then
                        ...
                        if dist < 2 then
                            ...
                            if IsControlJustPressed(0, 54) then
                                TriggerServerEvent('sunny:labs:exit', v.id)
                            end
                        end
                    end
                end
    ...
    RegisterNetEvent('sunny:labs:exit', function(type, exitCoords, id)
        _LabsSystem.isInLab = false
    ...
    RegisterCommand('labs', function()
        TriggerServerEvent('sunny:labs:enter', 2)
    end)
end)