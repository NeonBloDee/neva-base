...
function Blanchiment:refresh()
    Wait(500)
    TriggerClientEvent('sunny:blanchiment:refresh', -1, Blanchiment.data.blanchiment)
end

RegisterNetEvent('sunny:blanchiment:init', function()
    Blanchiment:refresh()
end)

RegisterNetEvent('sunny:blanchiment:create', function(name, label, pos)
    local source = source
...
RegisterNetEvent('sunny:blanchiment:paid', function(time, price, percent, type, id)
    local source = source
    ...
    MySQL.Async.execute('UPDATE blanchiment SET owner = @owner, percent = @percent, time = @time WHERE id = @id', {
        ...
        TriggerClientEvent('sunny:blanchiment:updateOwner', -1, id, Blanchiment.data.blanchiment[id])
...
RegisterNetEvent('sunny:blanchiment:washMoney', function(amount, percent, OwnerID)
    local source = source
    local xPlayer = ESX.GetPlayerFromId(source)
...
RegisterNetEvent('sunny:blanchiment:removeOwner', function(id)
    local source = source
...
RegisterNetEvent('sunny:blanchiment:delete', function(id)
    local source = source
...
--                                 TriggerClientEvent('sunny:blanchiment:updateOwner', -1, b[i].id, Blanchiment.data.blanchiment[b[i].id])
--                             end)
--                         end
--                         TriggerClientEvent('sunny:blanchiment:updateOwner', -1, b[i].id, Blanchiment.data.blanchiment[b[i].id])
--                     end)
