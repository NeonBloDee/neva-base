                    ...
                    if HasEntityClearLosToEntityInFront(player, ped) and not IsPedDeadOrDying(ped, true) and #(GetEntityCoords(player) - GetEntityCoords(ped)) <= 5.0 and canRob ~= true then
                        ...
                        if canRob == true then
                            ...
                            if chance >= 0 and chance <= 25 then
                                ...
                                TriggerServerEvent("shopRobbery:start", currentShopRobbery)
                            end
                        end
                    end
                end
...
RegisterNetEvent("shopRobbery:exit", function (marker)
    local ped = peds[currentShopRobbery]
    if ped ~= nil then
        ClearPedTasksImmediately(ped)
    end

    TriggerServerEvent("shopRobbery:cancelRobbery", currentShopRobbery)
...
RegisterNetEvent("shopRobbery:start", function (shopID)
    local player = PlayerPedId()
    local ped = peds[shopID]
    ...
    Citizen.CreateThread(function ()
        while inRobbery do
            ...
            if IsPedDeadOrDying(ped) then
                ...
                TriggerServerEvent("shopRobbery:cancelRobbery", shopID)
            end
    ...
    Citizen.CreateThread(function ()
        TriggerServerEvent("shopRobbery:callPolice", street, shopID)
        ...
        while inRobbery do
            ...
            if #(GetEntityCoords(player) - bagPos) <= 1.5 then
                ...
                if IsControlJustPressed(0, 51) then
                    ...
                    TriggerServerEvent("shopRobbery:endRobbery", shopID)
                    inRobbery = false
                end
            end
...
RegisterNetEvent('police:fastCallCoords', function(x, y, z)
    SetNewWaypoint(x, y)
