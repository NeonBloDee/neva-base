...
RegisterServerEvent('sunny:bank:braquage:toofar')
AddEventHandler('sunny:bank:braquage:toofar', function(robb)
	local source = source
	...
	for i=1, #xPlayers, 1 do
 		...
 		if xPlayer.job.name == 'police' or xPlayer.job.name == 'bcso' then
			...
			TriggerClientEvent('sunny:bank:braquage:killblip', xPlayers[i])
		end
	end
	if(robbers[source])then
		TriggerClientEvent('sunny:bank:braquage:toofarlocal', source)
		robbers[source] = nil
		TriggerClientEvent('esx:showNotification', source, _U('robbery_has_cancelled') .. Banks[robb].nameofbank)
	end
end)

RegisterServerEvent('sunny:bank:braquage:rob')
AddEventHandler('sunny:bank:braquage:rob', function(robb)
			...
			if(cops >= 1)then
				...
				for i=1, #xPlayers, 1 do
					...
					if xPlayer.job.name == 'police' or xPlayer.job.name == 'bcso' then
                        ...
                        TriggerClientEvent('sunny:bank:braquage:setblip', xPlayers[i], Banks[robb].position)
					end
				end
				...
				TriggerClientEvent('sunny:bank:braquage:currentlyrobbing', source, robb)
				...
				SetTimeout(300000, function()
					...
					if(robbers[savedSource])then
						...
						rob = false
						TriggerClientEvent('sunny:bank:braquage:robberycomplete', savedSource, job)
						if(xPlayer)then
