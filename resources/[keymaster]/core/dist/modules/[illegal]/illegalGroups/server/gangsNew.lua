...
function Gangs:delete()
    ...
    TriggerClientEvent('sunny:gangs:delete', -1, self.name)
    ...
    MySQL.Async.execute('INSERT INTO gangs_new (name, label, positions, permissions, circuit, chest, blips, cat, blipscolor) VALUES (@name, @label, @positions, @permissions, @circuit, @chest, @blips, @cat, @blipscolor)', {
        ...
        MySQL.Async.execute('INSERT INTO jobs (name, label, type) VALUES (@name, @label, @type)', {
            ...
            MySQL.Async.execute('INSERT INTO job_grades (job_name, grade, name, label) VALUES (@job_name, @grade, @name, @label)', {
                ...
                TriggerClientEvent('sunny:gangs:recieveGangs', -1, Gangs.List)
...
function Gangs:editInformations(t, data, JSON)
    ...
    MySQL.Async.execute(('UPDATE gangs_new SET %s = @data WHERE name = @name'):format(t), {
        ...
        TriggerClientEvent('sunny:gangs:updateGang', -1, self.name, self)
    end)
end
...
function Gangs:addAccountMoney(account, money)
    ...
    TriggerClientEvent('sunny:gangs:updateChest', -1, self.name, self.chest)
...
RegisterNetEvent('sunny:gangs:create', function(data)
    Gangs:create(data)
end)

RegisterNetEvent('sunny:gangs:staff:edit', function(data, name, label, JSON)
...
RegisterNetEvent('sunny:gangs:staff:delete', function(gangName)
...
RegisterNetEvent('sunny:gangs:gestion:grades:create', function(gangName, gradeData)
    local gang = Gangs:get(gangName)
...
RegisterNetEvent('sunny:gangs:gestion:grades:delete', function(gangName, gradeName)
    local gang = Gangs:get(gangName)
...
RegisterNetEvent('sunny:gangs:gestion:grades:editGradePermissions', function(gangName, permissions)
    local gang = Gangs:get(gangName)
...
RegisterNetEvent('sunny:gangs:gestion:grades:editLabel', function(gangName, gradeName, newGradeLabel)
    local gang = Gangs:get(gangName)
...
ESX.RegisterServerCallback('sunny:gangs:gestions:members:fetch', function(source, cb)
    local source = source
...
RegisterNetEvent('sunny:gangs:manage:demotePlayer', function(targetUniqueID)
    local source = source
...
ESX.RegisterServerCallback('sunny:gangs:interaction:requestPlayerInventory', function(source, cb, target)
...
RegisterNetEvent('sunny:gangs:interaction:cagoulePlayer', function(target)
    local source = source
    local player = ESX.GetPlayerFromId(source)
    ...
    if not Gangs.CagoulePlayers[target] then
        ...
        TriggerClientEvent('sunny:gangs:interaction:cagoulePlayer', target, true)
        ...
        TriggerClientEvent('sunny:gangs:interaction:cagoulePlayer', target, false)
...
RegisterNetEvent('sunny:gangs:interaction:recruitPlayer', function(target, gangName, grade)
    local player = ESX.GetPlayerFromId(target)

    if not player then return end

    player.setJob2(gangName, grade)
end)

RegisterNetEvent('sunny:gangs:interaction:removePlayer', function(target, gangName, grade)
    local player = ESX.GetPlayerFromId(target)
...
RegisterNetEvent('sunny:gangs:chest:accounts:add', function(gangName, account, count)
    local source = source
    ...
    TriggerClientEvent('esx:showNotification', source, ('Vous avez pris %s%s$~s~ du coffre de votre groupe illégal'):format(SunnyConfigServ.GTACOLOR, count))
...
RegisterNetEvent('sunny:gangs:chest:accounts:remove', function(gangName, account, count)
    local source = source
    ...
    TriggerClientEvent('esx:showNotification', source, ('Vous avez retiré %s%s$~s~ du coffre de votre groupe illégal'):format(SunnyConfigServ.GTACOLOR, count))
...
RegisterNetEvent('sunny:gangs:chest:items:add', function(gangName, v, number)
    local source = source
    ...
    if not SunnyConfigServ.PermanentItem[v.name] then
...
RegisterNetEvent('sunny:gangs:chest:items:remove', function(gangName, v, number)
    local source = source
...
RegisterNetEvent('sunny:gangs:chest:weapons:add', function(gangName, v, number)
    local source = source
    ...
    gang:addInventoryItem(string.lower(v), 1)
...
AddEventHandler('sunny:admin:restart', function()
    ...
    TriggerClientEvent('sunny:gangs:recieveGangs', source, Gangs.List)
end)

RegisterNetEvent('sunny:gangs:chest:weapons:remove', function(gangName, v)
    local source = source
    local player = ESX.GetPlayerFromId(source)
    ...
    if retval then
        player.addInventoryItem(string.lower(v.name), 1)
