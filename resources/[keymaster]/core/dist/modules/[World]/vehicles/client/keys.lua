local lastInteraction = 0 -- Variable pour mémoriser le temps de la dernière interaction
...
local function OpenVehiclegARAGE()
	CreateThread(function()
		...
		if dist > 10 then
			...
			if currentTime - lastInteraction < 1500 then -- Si moins de 1,5 seconde s'est écoulée
					ESX.ShowNotification("⏳ Veuillez patienter avant de réessayer.")
					return
			end

			ESX.TriggerServerCallback('sunny:garage:requestPlayerCars', function(isOwnedVehicle)
				...
				if isOwnedVehicle then
						local currentTime = GetGameTimer() -- Récupère l'heure actuelle du jeu
						if currentTime - lastInteraction < 1500 then -- Si moins de 1,5 seconde s'est écoulée
								ESX.ShowNotification("⏳ Veuillez patienter avant de réessayer.")
								return
						end
						...
						-- Mise à jour de lastInteraction pour le délai de 1,5 seconde
