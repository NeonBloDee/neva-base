RegisterNetEvent('sunny:garage:spawnVehiclePound', function(vehicle, spawnPoint)
...
RegisterNetEvent('sunny:garages:edit:blips', function(value, id)
    if value == 'add' then
        ...
        AddTextComponentString(Garages.garage_data.garages[id].type == 'car' and 'Parking Terrestre' or Garages.garage_data.garages[id].type == 'boat' and 'Parking Nautique' or Garages.garage_data.garages[id].type == 'aircraft' and 'Parking Aérien')
...
RegisterNetEvent('sunny:garages:setvehicleProperties', function(vehicleNetworkID, tuning, entityId, plate)
    ...
    TriggerServerEvent('sunny:garages:syncEntityId', GetVehicleNumberPlateText(vehicle), entityId)
end)


RegisterNetEvent("sunny:vehicle:setFuel", function (vehicleNetworkID, fuel)
    while not NetworkDoesEntityExistWithNetworkId(vehicleNetworkID) do Citizen.Wait(200) end
...
AddEventHandler("player:enteredDriverVehicle", function (vehicle)
    ...
    if playerData.job.name == "police" or playerData.job.name == "ambulance" or playerData.job.name == "saspn" or playerData.job.name == "gouvernement" then return end
