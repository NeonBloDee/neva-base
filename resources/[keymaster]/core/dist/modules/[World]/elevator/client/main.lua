...
function AsxenseuropenAcceuilMenu(job, message, label)
   local main = RageUI.CreateMenu('', 'Acceuil')
    ...
    while main do Wait(1)
        RageUI.IsVisible(main, function()
            RageUI.Button('Appeler un membre du personnel '..label, nil, {}, not SunnyConfigServ.Elevator.acceuil.cooldown, {
                onSelected = function()
                    TriggerServerEvent('sunny:acceuil:sendAnnounce', job, message)

                    SunnyConfigServ.Elevator.acceuil.cooldown = true
                    SetTimeout(120000, function()
                        SunnyConfigServ.Elevator.acceuil.cooldown = false
                    end)
                end
            })
        end)
...
CreateThread(function()
    ...
    while true do
        ...
        for k,v in pairs(SunnyConfigServ.Elevator.positions) do
            ...
            if dist < 1 then
                ...
                if IsControlJustPressed(1,51) then
                    AsxenseuropenMenu(SunnyConfigServ.Elevator.positions, v.t)
                end
            end
        ...
        for k,v in pairs(SunnyConfigServ.Elevator.acceuil.pos) do 
