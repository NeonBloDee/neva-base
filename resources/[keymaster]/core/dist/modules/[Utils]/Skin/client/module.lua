...
function ApplySkin(skin, clothes)
	...
	-- Fix for SetPedEyeColor native - ensure eye_color is a valid number
	if Character['eye_color'] ~= nil and type(Character['eye_color']) == "number" then
		SetPedEyeColor				(playerPed,			Character['eye_color'], 0, 1)												-- Eyes color
	else
		SetPedEyeColor				(playerPed,			0, 0, 1)												-- Default Eyes color
	end
...
Citizen.CreateThread(function()
    ...
    TriggerEvent("Sunny:skinchanger:sendBagConfig", bagsWeight)
