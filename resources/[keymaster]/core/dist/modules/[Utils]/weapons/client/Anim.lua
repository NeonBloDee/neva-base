...
AddEventHandler("NVTeam:skinchanger:sendBagConfig", function(data)
    bagList = data
end)
			...
			if newWeap ~= currWeapon then
				...
				if continue then
					...
					if (weapons[newWeap] ~= nil) and (weapons[newWeap].parameters.anim) then
						if holstered then
							canFire = false
							SunnyHolsters[Player.holsterPreset].intro(Player.playerPed, newWeap, function()
								holstered = false
								canFire = true
								currWeapon = newWeap
							end)
						elseif newWeap ~= currWeapon then
							canFire = false
							SunnyHolsters[Player.holsterPreset].outro(Player.playerPed, newWeap)
							Citizen.Wait(1000)
							SunnyHolsters[Player.holsterPreset].intro(Player.playerPed, newWeap, function()
								holstered = false
								canFire = true
						...
						if not holstered and (weapons[currWeapon] ~= nil) and (weapons[currWeapon].parameters.anim) then
							canFire = false
							SunnyHolsters[Player.holsterPreset].outro(Player.playerPed, newWeap, function()
								holstered = true
								canFire = true
							end)
							SetCurrentPedWeapon(Player.playerPed, unarmedHash, true)
...
local function canAim()
	if IsEntityDead(Player.playerPed) then
		...
		for i = 1, #SunnyAimWeapons do
			if GetHashKey(SunnyAimWeapons[i]) == Player.weapon then
				return true
			end
		end
...
local showWeaponsInBack = SunnyConfigServ.WeaponInBack.showWeaponsInBack
Citizen.CreateThread(function()
    while true do
        local me = PlayerPedId()
        local selectedWeapon = GetSelectedPedWeapon(me)
        if showWeaponsInBack == true then
        for wep_name, wep_hash in pairs(SunnyConfigServ.WeaponInBack.compatable_weapon_hashes) do
            if HasPedGotWeapon(me, wep_hash, false) and not attached_weapons[wep_name] and selectedWeapon ~= wep_hash then
                AttachWeapon(wep_name, wep_hash, SunnyConfigServ.WeaponInBack.back_bone, SunnyConfigServ.WeaponInBack.x, SunnyConfigServ.WeaponInBack.y, SunnyConfigServ.WeaponInBack.z, SunnyConfigServ.WeaponInBack.x_rotation, SunnyConfigServ.WeaponInBack.y_rotation, SunnyConfigServ.WeaponInBack.z_rotation, isMeleeWeapon(wep_name))
            end
        end
	end
