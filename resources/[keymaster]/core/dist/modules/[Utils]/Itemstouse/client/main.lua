...
RegisterCommand('nitroVehicle', function(source,args)
    ...
    if (vehicle ~= 0) then 
        TriggerServerEvent('sunny:useNitro', NetworkGetNetworkIdFromEntity(vehicle))
    end
end)
RegisterKeyMapping('nitroVehicle', 'Activer Nitro Véhicule', 'keyboard', 'O')


RegisterNetEvent('sunny:useciseaux', function()
    if not GetSafeZone() then
        ...
        if closestPlayer ~= -1 and closestDistance <= 3.0 then
            ...
            Wait(10000)
            DeleteObject(ciseau)
            TriggerServerEvent('sunny:haircut', GetPlayerServerId(closestPlayer))
...
RegisterNetEvent('sunny:haircut', function()
