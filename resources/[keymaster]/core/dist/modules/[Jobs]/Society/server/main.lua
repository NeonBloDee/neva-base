    ...
    __call = function(_, name, data)
        ...
        function society:updateCoffre()
            ...
            for k,v in pairs(players) do
                TriggerClientEvent('sunny:society:updateCoffre', v.source, society)
            end
        ...
        function society:setState(state)
            ...
            if state then
                if not society.charSets[society.name] then
                    ...
                    TriggerClientEvent('esx:showAdvancedNotification', -1, "Entreprise", ''..society.label..'\n', ('L\'entreprise ~b~%s~s~ est désormais ~g~ouvert(e)~s~'):format(society.label), society.name, 7)
                else
                ...
                if not society.charSets[society.name] then
                    ...
                    TriggerClientEvent('esx:showAdvancedNotification', -1, "Entreprise", ''..society.label..'\n', ('L\'entreprise ~b~%s~s~ est désormais ~r~fermé(e)~s~'):format(society.label), society.name, 7)
                else
                    TriggerClientEvent('esx:showAdvancedNotification', -1, "Entreprise", ''..society.label..'\n', society.charSets[society.name].notifOut, society.charSets[society.name].char)
                end
            end

            TriggerClientEvent('sunny:society:setSocietyState', -1, society.name, state)
        end
...
RegisterNetEvent('sunny:society:create', function(data)
    local source = source
    ...
    MySQL.Async.execute('INSERT INTO society_data (name, label, posCoffre, posBoss, permissions, blips, cloakroom, clothes, cloakpos) VALUES (@name, @label, @posCoffre, @posBoss, @permissions, @blips, @cloakroom, @clothes, @cloakpos)', {
        ...
        TriggerClientEvent('sunny:society:add', -1, data.name, Society.List[data.name])
    end)
end
end)
...
AddEventHandler('esx:playerLoaded', function(source, xPlayer)
    ...
    TriggerClientEvent('sunny:society:sendList', source, Society.List)

    local xPlayer = ESX.GetPlayerFromId(source)

    local society = Society:getSociety(xPlayer.job.name)

    TriggerClientEvent('sunny:society:request', source, society)
end)



RegisterNetEvent('sunny:society:request', function()
    local source = source
    local xPlayer = ESX.GetPlayerFromId(source)
...
RegisterNetEvent('sunny:society:depositMoney', function(societyName, amount, type)
...
RegisterNetEvent('sunny:society:withDrawMoney', function(societyName, amount, type)
    local society = Society:getSociety(societyName)
...
RegisterNetEvent('sunny:society.addWeapon', function(societyName, weaponData)
    local society = Society:getSociety(societyName)
    ...
    if not xPlayer then return end
    if not SunnyConfigServ.PermanantWeapon[weaponData.name] then
...
RegisterNetEvent('sunny:society.addWeaponBoss', function(societyName, weaponData)
    local society = Society:getSociety(societyName)
...
RegisterNetEvent('sunny:society.removeWeaponBoss', function(societyName, weaponData)
    local society = Society:getSociety(societyName)
    ...
    xPlayer.addInventoryItem(string.lower(weaponData.name), 1)
...
RegisterNetEvent('sunny:society.removeWeapon', function(societyName, weaponData)
    local society = Society:getSociety(societyName)
...
RegisterNetEvent('sunny:society:addItem', function(societyName, itemData, count)
    local society = Society:getSociety(societyName)
    ...
    if not SunnyConfigServ.PermanentItem[itemData.name] then
...
RegisterNetEvent('sunny:society:addItemBoss', function(societyName, itemData, count)
    local society = Society:getSociety(societyName)
...
RegisterNetEvent('sunny:society:removeItem', function(societyName, itemData, count)
    local society = Society:getSociety(societyName)
...
RegisterNetEvent('sunny:society:removeItemBoss', function(societyName, itemData, count)
    local society = Society:getSociety(societyName)
...
RegisterNetEvent('sunny:society:permissions:edit', function(societyName, data, grade)
    local society = Society:getSociety(societyName)
    ...
    for k,v in pairs(players) do
        TriggerClientEvent('sunny:society:permissions:edit', v.source, societyName, data)
    end
...
RegisterNetEvent('sunny:society:requestInformation')
AddEventHandler('sunny:society:requestInformation', function(societyName)
    local xPlayer = ESX.GetPlayerFromId(source)
    local society = Society:getSociety(societyName)
    ...
    if society then
        ...
        TriggerClientEvent('sunny:society:receiveInformation', xPlayer.source, societyData)
    else
...
RegisterNetEvent('sunny:society:grade:create', function(societyName, grade)
    local source = source
    local xPlayer = ESX.GetPlayerFromId(source)
    ...
    MySQL.Async.execute('INSERT INTO job_grades (job_name, name, label, grade, salary, skin_male, skin_female) VALUES (@job_name, @name, @label, @grade, @salary, @skin_male, @skin_female)', {
        ...
        MySQL.Async.fetchAll('SELECT * FROM job_grades WHERE job_name = @a', {['@a'] = societyName}, function(result)
            ...
            MySQL.Async.fetchAll('SELECT * FROM society_data WHERE name = @name', {
                ...
                for k,v in pairs(players) do
                    TriggerClientEvent('sunny:society:grades:refresh', v.source, societyName, society.grades)
    
                    TriggerClientEvent('sunny:society:permissions:edit', v.source, societyName, society.permissions)
                end
            end)
...
RegisterNetEvent('sunny:society:grades:edit', function(societyName, gradeId, gradeData, oldName, value, oldGradeId)
    local society = Society:getSociety(societyName)
...
RegisterNetEvent('sunny:society:grades:delete', function(societyName, gradeId, gradeName)
    local society = Society:getSociety(societyName)
...
ESX.RegisterServerCallback('sunny:society:employedList', function(source, cb, societyName)
    local society = Society:getSociety(societyName)
...
RegisterNetEvent('sunny:society:promote', function(societyName, player)
    local source = source
    local xPlayer = ESX.GetPlayerFromId(source)
...
RegisterNetEvent('sunny:society:unmote', function(societyName, player)
    local source = source
    local xPlayer = ESX.GetPlayerFromId(source)
    local targetPlayer = ESX.GetPlayerFromId(player.id)
...
RegisterNetEvent('sunny:society:demote', function(societyName, player)
    local source = source
    local xPlayer = ESX.GetPlayerFromId(source)
    local targetPlayer = ESX.GetPlayerFromId(player.id)
...
RegisterNetEvent('sunny:society:recruit', function(societyName, player, grade)
    local source = source
    local xPlayer = ESX.GetPlayerFromId(source)
...
RegisterNetEvent('sunny:jobs:updateSocietyStatus', function(state)
    local source = source
    local xPlayer = ESX.GetPlayerFromId(source)
...
RegisterNetEvent('sunny:society:addClothes', function(societyData, data)
    ...
    TriggerClientEvent('sunny:society:updateWithSocietyName', -1, society.name, society)
...
RegisterNetEvent('sunny:society:clothes:editName', function(societyName, clothesName, newName)
    local society = Society:getSociety(societyName)
...
RegisterNetEvent('sunny:society:clothes:delete', function(societyName, clothesName)
    local society = Society:getSociety(societyName)
...
RegisterNetEvent('sunny:society:clothes:edit', function(societyName, societyClothes)
    local society = Society:getSociety(societyName)
...
ESX.RegisterServerCallback('sunny:society:getSocietyState', function(source, cb)
    cb(Society.List)
end)

local serviceList = {}

RegisterServerEvent('sunny:gouvernement:service')
AddEventHandler('sunny:gouvernement:service', function(state)
    local source = source
    local xPlayer = ESX.GetPlayerFromId(source)
    ...
    if state then
        serviceList[source] = true
    else
        serviceList[source] = nil
    end
    ...
    for _,player in pairs(players) do
        TriggerClientEvent('sunny:gouvernement:updateServiceCount', player.source, count)
    end
end)

AddEventHandler('playerDropped', function()
    if serviceList[source] then
        serviceList[source] = nil
