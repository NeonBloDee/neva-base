...
RegisterNetEvent('sunny:taxi:service', function(newVal)
    taxi.Service = newVal
end)

RegisterNetEvent('sunny:taxi:addCall', function(k,v)
    taxi.Appelles[k] = v
...
RegisterNetEvent('sunny:taxi:removeCall', function(k)
    taxi.Appelles[k] = nil

    taxi.AppellesCount -= 1
end)

RegisterNetEvent('sunny:taxi:updateCall', function(k, v)
    taxi.Appelles[k] = v
end)

RegisterNetEvent('sunny:taxi:addEmployed', function(k,v)
    taxi.Employeds[k] = v
...
RegisterNetEvent('sunny:taxi:removeEmployed', function(k)
...
RegisterNetEvent('sunny:taxi:takeMyCall', function(k)
    ...
    while main do Wait(1)
        RageUI.IsVisible(main, function()
            ...
            RageUI.Checkbox('Service', nil, taxi.Service, {}, {
                onChecked = function()
                    TriggerServerEvent('sunny:taxi:service')
                end,
                onUnChecked = function()
                    TriggerServerEvent('sunny:taxi:service')
                end
                ...
                RageUI.Button('Faire une facture', nil, {}, true, {
                    onSelected = function()
                        ...
                        KeyboardUtils.use('Montant', function(data)
                            ...
                            TriggerServerEvent('esx_billing:sendBill', GetPlayerServerId(player), ('society_%s'):format(ESX.GetPlayerData().job.name), ESX.GetPlayerData().job.label, data)
                        end)
                    end
                })
                RageUI.Line()
        ...
        RageUI.IsVisible(sAppeles, function()
            if taxi.Appelles[taxi.sAppele] ~= nil then
                RageUI.Button('Prendre', nil, {}, taxi.Appelles[taxi.sAppele].take ~= true, {
                    onSelected = function()
                        taxi:RemoveBlip()
                        TriggerServerEvent('sunny:taxi:takeAppel', taxi.sAppele)
                    end
                })
                RageUI.Button('Finir/Supprimer', nil, {}, true, {
                    onSelected = function()
                        TriggerServerEvent('sunny:taxi:closeAppel', taxi.sAppele)
                        taxi:RemoveBlip()
                    end
                })
            else
		...
		Citizen.CreateThread(
			function()
				...
				TriggerEvent(
					...
					SunnyConfigServ.GTACOLOR.."Demande de transport.",
...
--                     onSelected = function()
--                         TriggerServerEvent('sunny:taxi:buyitem', v.name)
--                     end
--                 })
--             end
--         end)
...
--                         TriggerServerEvent('sunny:taxi:moneyBoss', ('society_%s'):format("taxi"), data, 'deposit')
--                     end)
--                 end
--             })
...
--                         TriggerServerEvent('sunny:taxi:moneyBoss', ('society_%s'):format("taxi"), data, 'remove')
--                     end)
--                 end
--             })
--         end)
...
--     AddZones('TaxiZone', {
--         Position = SunnyGlobal.TaxiJob.PosShopTaxi,
--         Dist = 10,
--         Public = false,
--         Job = {['taxi'] = true},
--         Job2 = nil,
