...
local JeSuiSMort = false
local rea = false
local SetEntityRagdoll = false
local menumortisopen = false
local deathReason

RegisterNetEvent("sunny:ambulance:respawnPed", function(spawn)  --- TODO
...
CreateThread(function()
    while true do 
        timer = 1000
        if SetEntityRagdoll == true then 
            if IsPedRagdoll(PlayerPedId()) == false and JeSuiSMort == true then 
                timer = 0
...
CreateThread(function()
    while true do 
        ...
        if JeSuiSMort == true then 
            waittime = 400
            if IsPedRagdoll(PlayerPedId()) == false then 
                SetEntityRagdoll = true
            end

            if menumortisopen == false then 
...
RegisterNetEvent('ata:client:update', function(ata)  ---- TODO
    if ata == nil then 
        return 
    end
...
RegisterNetEvent('sunny:ambulance:service', function(newVal)
    Ambulance.Service = newVal
end)

RegisterNetEvent('sunny:ambulance:addCall', function(k,v)
...
RegisterNetEvent('sunny:ambulance:removeCall', function(k)
    Ambulance.Appelles[k] = nil

    Ambulance.AppellesCount -= 1
end)

RegisterNetEvent('sunny:ambulance:updateCall', function(k, v)
    Ambulance.Appelles[k] = v
end)

RegisterNetEvent('sunny:ambulance:addEmployed', function(k,v)
        ...
        RageUI.IsVisible(maindefibrilateur, function()
            ...
            if player ~= -1 and dist < 5 then 
                ...
                RageUI.Button("→ ~g~Nom : ~w~"..GetPlayerName(player), "Prix : 1200$", {RightLabel = "Réanimer"}, true, {
                    onSelected = function()
                        TriggerServerEvent("sunny:reamoi", GetPlayerServerId(player))
                        RageUI.CloseAll()
                    end
                })
            else
...
RegisterNetEvent('sunny:ambulance:removeEmployed', function(k)
    Ambulance.Employeds[k] = nil
    Ambulance.EmployedsCount = 0

    Ambulance.EmployedsCount -= 1
end)
RegisterNetEvent("sunny:ambulance:stungun", function()  --- TODO
    Citizen.CreateThread(function()
    ...
    if eventName == "CEventNetworkEntityDamage" then
        ...
        if not weaponUsed then
            ...
            if not PLAYERDEADLEGIT then 
                ...
                TriggerEvent("sunny:ambulance:respawnPed", {coords = GetEntityCoords(PlayerPedId()), heading = 0.0})  --- TODO
            end
        end

        if weaponUsed == `WEAPON_SHOTGUNLETAL` or weaponUsed == `WEAPON_STUNGUN` then
            if GetEntityHealth(PlayerPedId()) < 100 then
                TriggerEvent("sunny:ambulance:respawnPed", {coords = GetEntityCoords(PlayerPedId()), heading = 0.0})
            end
            TriggerServerEvent("sunny:ambulance:stungun")  --- TODO
            return
        end
            ...
            if (GetWeaponDamageType(weaponUsed) == 2) and weaponUsed ~= -1716189206 then 
                ...
                if not PLAYERDEADLEGIT then 
                    ...
                    if (Killer == nil) or (Killer == Player.playerId) then
                        TriggerServerEvent("sunny:ambulance:playerDiedWithoutReason")
                    else
                        TriggerServerEvent("sunny:ambulance:playerDiedWithReason")
                    end
                else
                    TriggerEvent("sunny:ambulance:respawnPed", {coords = GetEntityCoords(PlayerPedId()), heading = 0.0})
                end
...
RegisterNetEvent('sunny:ambulance:takeMyCall', function(k)
        ...
        if RageUI.Visible(main) then
            RageUI.IsVisible(main, function()
                ...
                RageUI.Checkbox('Service', nil, Ambulance.Service, {}, {
                    onChecked = function()
                        TriggerServerEvent('sunny:ambulance:service')
                    end,
                    onUnChecked = function()
                        TriggerServerEvent('sunny:ambulance:service')
                    end
                ...
                if Ambulance.Appelles[Ambulance.sAppele] ~= nil then
                    RageUI.Button('Prendre', nil, {}, Ambulance.Appelles[Ambulance.sAppele].take ~= true, {
                        onSelected = function()
                            ...
                            TriggerServerEvent('sunny:ambulance:takeAppel', Ambulance.sAppele)
                        end
                    })
                    RageUI.Button('Finir/Supprimer', nil, {}, true, {
                        onSelected = function()
                            TriggerServerEvent('sunny:ambulance:closeAppel', Ambulance.sAppele)
                            Ambulance:RemoveBlip()
                        end
                    })
                else
                    ...
                    onSelected = function()
                        if closestDistance ~= -1 and closestDistance <= 3 then
                            ...
                            drawBar(8000, 'Réanimation de la personne en cours', function()
                                TriggerServerEvent('sunny:ambulance:revive', GetPlayerServerId(closestPlayer))
                    ...
                    onSelected = function()
                        if closestDistance ~= -1 and closestDistance <= 3 then
                            ...
                            drawBar(8000, 'Soins de la personne en cours', function()
                                TriggerServerEvent('sunny:ambulance:heal', GetPlayerServerId(closestPlayer), 'p')
                                ClearPedTasks(playerPed)
                ...
                RageUI.Button('Soigner une grande blessure', nil, {}, true, {
                    onSelected = function()
                        if closestDistance ~= -1 and closestDistance <= 3 then
                            ...
                            TaskStartScenarioInPlace(playerPed, 'CODE_HUMAN_MEDIC_TEND_TO_DEAD', 0, true)
                            drawBar(8000, 'Soins de la personne en cours', function()
                                TriggerServerEvent('sunny:ambulance:heal', GetPlayerServerId(closestPlayer), 'g')
                                ClearPedTasks(playerPed)
...
AddEventHandler('esx:onPlayerDeath', function()
    ...
    TriggerServerEvent('sunny:deadlogs:send', GetPlayerServerId(Killer))
end)
...
--     TriggerServerEvent('sunny:ambulance:call', GetEntityCoords(Player.playerPed))
...
RegisterNetEvent('sunny:ambulance:deathEvent', function()
    ...
    if IsEntityAPed(PedKiller) and IsPedAPlayer(PedKiller) then
        ...
        ESX.TriggerServerCallback('sunny:ambulance:getKilerID', function(bbbb)
            KillerUniqueId = bbbb
    ...
    RegisterNUICallback("deathscreen:respawn", function()
        ...
        TriggerEvent("deathscreen:close")
    ...
    RegisterNUICallback("deathscreen:report", function()
        if deathReason ~= "Vous avez été tué par quelqu'un" then 
            ESX.ShowNotification('Vous ne vous êtes pas fait tuer par un joueur')
            return
        end
    ...
    SendNUIMessage({
        type = "death:info",
        reason = deathReason,
        killerId = KillerUniqueId or "Aucun"
    })
end)
...
Citizen.CreateThread(function()
    while true do
        ...
        if JeSuiSMort and SetEntityRagdoll then
            waitTime = 0
            local playerPed = PlayerPedId()
...
RegisterNetEvent('sunny:admin:revive', function()
    JeSuiSMort = false
    SetEntityRagdoll = false
    ...
    TriggerServerEvent('sunny:ambulance:sql:removedead')
    DoScreenFadeIn(800)
    Citizen.CreateThread(function()
        RespawnPed(playerPed, {coords = coords, heading = 0.0})
        AnimpostfxStop('DeathFailOut')
...
RegisterNetEvent('sunny:ambulance:revive', function()
    JeSuiSMort = false
    SetEntityRagdoll = false
...
RegisterNetEvent('sunny:ambulance:heal', function(type)
...
--     TriggerServerEvent('sunny:ambulance:deathReturn')
        ...
        RageUI.IsVisible(main, function()
            for k,v in pairs(Ambulance.Items) do
                RageUI.Button(v.label, nil, {RightLabel = '~g~GRATUIT~s~'}, true, {
                    onSelected = function()
                        TriggerServerEvent('sunny:ambulance:buyitem', v.name)
                    end
                })
            end
        end)
            ...
            RageUI.Button('Déposer de l\'argent', 'Argent en banque', {}, true, {
                onSelected = function()
                    KeyboardUtils.use('Montant à déposer', function(data)
                        ...
                        TriggerServerEvent('sunny:ambulance:moneyBoss', ('society_%s'):format("ambulance"), data, 'deposit')
                    end)
                end
            })
            RageUI.Button('Prendre de l\'argent', 'Argent du coffre', {}, true, {
                onSelected = function()
                    KeyboardUtils.use('Montant à prendre', function(data)
                        ...
                        TriggerServerEvent('sunny:ambulance:moneyBoss', ('society_%s'):format("ambulance"), data, 'remove')
                    end)
                end
            })
        end)
...
RegisterNetEvent('sunny:ambulance:deathEvent', function()
    ...
    local Weapon = ESX.GetWeaponHash(DeathCauseHash) == nil and "Aucune" or ESX.GetWeaponHash(DeathCauseHash).label or "Aucune"
