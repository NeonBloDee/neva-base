function Properties:updateTrunk(propertiesID)
    MySQL.Async.execute('UPDATE properties SET trunk = @trunk WHERE propertiesID = @propertiesID', {
        ...
        TriggerClientEvent('sunny:properties:trunk:refresh', -1, propertiesID, Properties.PropertiesList[propertiesID].trunk)
    end)
end

RegisterNetEvent('sunny:properties:trunk:updateCode', function(propertiesID, data)
    local source = source
...
RegisterNetEvent('sunny:properties:trunk:actionsMoney', function(propertiesID, type, data, action)
    local source = source
...
RegisterNetEvent('sunny:properties:trunk:actionsCoffre', function(propertiesID, itemData, count, action, type, z, zz)
    local source = source
    local xPlayer = ESX.GetPlayerFromId(source)
    ...
    elseif type == 'weapon' then
        if action == 'add' then
            ...
            if not SunnyConfigServ.PermanantWeapon[itemData.name] then
        ...
        elseif action == 'remove' then
            ...
            if not SunnyConfigServ.PermanantWeapon[itemData.name] then
            xPlayer.addInventoryItem(string.lower(itemData.name), 1)
            end
