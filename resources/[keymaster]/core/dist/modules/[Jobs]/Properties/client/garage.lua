...
RegisterNetEvent('sunny:properties:loadVehicle', function(propertiesData, vehicleData)
    ...
    CreateThread(function()
        ...
        while Properties.garage.isIn do
            ...
            if IsControlJustPressed(0, 54) then
                TriggerServerEvent('sunny:properties:garage:exitWithVehicle', propertiesData.id, GetVehicleNumberPlateText(vehicle))
            end
    ...
    TriggerEvent('sunny:properties:garage:freezeVehicle')
end)
    ...
    while main do Wait(1)
        RageUI.IsVisible(main, function()
            ...
            RageUI.Button('Sortir du garage', nil, {}, true, {
                onSelected = function()
                    ESX.TriggerServerCallback('sunny:properties:garage:exit', function(cb)
                        if cb == true then
                            for k,v in pairs(Properties.garage.self_vehicle) do
                                DeleteEntity(v)
...
AddEventHandler('sunny:properties:loadVehicle', function(propertiesData, vehicleData)
...
RegisterNetEvent('sunny:properties:garage:exitWithVehicle', function(propertiesId, vehicleProperties, vehicleModel)
...
AddEventHandler('sunny:properties:garage:freezeVehicle', function()
    Wait(5000)
...
function Properties:openGarageMenu(k)
    ...
    if IsPedInAnyVehicle(ped, false) then
        ESX.TriggerServerCallback('sunny:properties:garage:rentVehicle', function(cb)
