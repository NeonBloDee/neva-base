...
function openUndriveableMenu(vehicle)
    ...
    TriggerServerEvent('sunny:checkVehiclePlate', vehiclePlate)
            ...
            RageUI.IsVisible(menu, function()
                if calledVehiclePlates[vehiclePlate] then
                    ...
                    RageUI.Button("Contacter un mécanicien", nil, {}, true, {
                        onSelected = function()
                            TriggerServerEvent("sunny:sendappel:job", {
                                job = "mecano",
...
RegisterNetEvent('sunny:checkVehiclePlateResult', function(plate, exists)
...
local LsCustoms = {
    Pos = {
        {pos = vector3(-208.511765, -1336.053467, 31.300480), job = "mecano", notif = "Appuyez sur ~INPUT_CONTEXT~ pour accéder au "..SunnyConfigServ.GTACOLOR.."Benny's~s~.", size = 20.0},
        {pos = vector3(-214.471405, -1335.609009, 31.301208), job = "mecano", notif = "Appuyez sur ~INPUT_CONTEXT~ pour accéder au "..SunnyConfigServ.GTACOLOR.."Benny's~s~.", size = 20.0},
...
RegisterNetEvent("sunny:mechanic:receivePlayerBillsState", function(bool)
    ...
    if (bool == true) then
        ...
        if (player ~= -1 and distance <= 3.0) then
            ...
            TriggerServerEvent('esx_billing:sendBill', GetPlayerServerId(player), 'society_'..jobName, (banana), math.floor(LsCustoms.Price*1.8));
            LsCustoms.openedMenu = false;
            RageUI.CloseAll();
...
RegisterNetEvent('sunny:mechanic:receivePlayerBillsState:patron', function(bool)
    if (bool == true) then
...
function MenuCustomization()
    LsCustoms.mainMenu = RageUI.CreateMenu(" ", "Mécano")
    LsCustoms.subMenuColour = RageUI.CreateSubMenu(LsCustoms.mainMenu, " ", "Mécano")
    LsCustoms.subMenuRoues = RageUI.CreateSubMenu(LsCustoms.mainMenu, " ", "Mécano")
    LsCustoms.subMenuClassiques = RageUI.CreateSubMenu(LsCustoms.mainMenu, " ", "Mécano")
    LsCustoms.subMenuCustoms = RageUI.CreateSubMenu(LsCustoms.mainMenu, " ", "Mécano")
    LsCustoms.subMenuPerf = RageUI.CreateSubMenu(LsCustoms.mainMenu, " ", "Mécano")
    LsCustoms.subMenuExtra = RageUI.CreateSubMenu(LsCustoms.mainMenu, " ", "Mécano")
    LsCustoms.subMenuModification = RageUI.CreateSubMenu(LsCustoms.mainMenu, " ", "Mécano")
            ...
            CreateThread(function()
                while LsCustoms.openedMenu do
                        ...
                        if LsCustoms.Price > 0 then
                            RageUI.Button("Valider les modifications", nil, {RightLabel = "~g~"..LsCustoms.Price.."$"}, true, {
                                onSelected = function()
                                    menucustomisopne = false
                                    local player, distance = ESX.Game.GetClosestPlayer();
                                    TriggerServerEvent("sunny:mechanic:requestPlayerBillsState", GetPlayerServerId(player), tonumber(LsCustoms.Price));
                                    RageUI.CloseAll()
                                end
                            })
                            ...
                            if ESX.GetPlayerData().job.grade_name == 'boss' then
                                RageUI.Button("Valider les modifications pour le patron", nil, {RightLabel = "~g~"..LsCustoms.Price.."$"}, true, {
                                    onSelected = function()
                                        menucustomisopne = false
                                        local player, distance = ESX.Game.GetClosestPlayer();
                                        TriggerServerEvent("sunny:mechanic:patron", tonumber(LsCustoms.Price));
                                        RageUI.CloseAll()
                                    end
                                })
                    ...
                    RageUI.IsVisible(LsCustoms.subMenuPerf, function()
                        ...
                        -- AJOUT DES NÉONS ICI
                        RageUI.Separator("~b~Options Néons")
                    
                        local neonPositions = {
                            { name = "Avant", index = 0 },
                            { name = "Arrière", index = 1 },
                            { name = "Gauche", index = 2 },
                            { name = "Droite", index = 3 }
                        }
                    
                        for _, neon in pairs(neonPositions) do
                            local isActive = IsVehicleNeonLightEnabled(LsCustoms.Vehicle, neon.index)
                            RageUI.Checkbox("Néon " .. neon.name, nil, isActive, {}, {
                                onChecked = function()
                                    SetVehicleNeonLightEnabled(LsCustoms.Vehicle, neon.index, true)
                                    LsCustoms.PriceNeon = (LsCustoms.PriceNeon or 0) + 5000
                                end,
                                onUnChecked = function()
                                    SetVehicleNeonLightEnabled(LsCustoms.Vehicle, neon.index, false)
                                    LsCustoms.PriceNeon = (LsCustoms.PriceNeon or 0) - 5000
                                end
                            })
                        end
                    
                        -- LISTE DES COULEURS DE NÉONS
                        local neonColors = {
                            ...
                            { label = "Violet", r = 128, g = 0, b = 128 },
                            { label = "Rose clair", r = 255, g = 182, b = 193 },
                            { label = "Rose foncé", r = 199, g = 21, b = 133 },
                            { label = "Orange", r = 255, g = 165, b = 0 },
                            { label = "Turquoise", r = 64, g = 224, b = 208 },
                            { label = "Cyan", r = 0, g = 255, b = 255 },
                            { label = "Bleu foncé", r = 0, g = 0, b = 139 },
                            { label = "Bleu clair", r = 173, g = 216, b = 230 },
                            { label = "Vert fluo", r = 57, g = 255, b = 20 },
                            { label = "Orange foncé", r = 255, g = 69, b = 0 },
                            { label = "Or", r = 255, g = 215, b = 0 },
                            { label = "Lime", r = 50, g = 205, b = 50 }
                        }
                        local r, g, b = GetVehicleNeonLightsColour(LsCustoms.Vehicle)
                        local currentColorIndex = 1
                        
                        for i, color in ipairs(neonColors) do
                        ...
                        RageUI.Button("Couleur des néons", "~o~" .. neonColors[currentColorIndex].label, { RightLabel = "→" }, true, {
                            onSelected = function()
                                -- Incrémentation de l'index
                                currentColorIndex = currentColorIndex + 1
                                if currentColorIndex > #neonColors then
                                    currentColorIndex = 1
                                end
                        
                                -- Appliquer la nouvelle couleur
                                local selectedColor = neonColors[currentColorIndex]
                                SetVehicleNeonLightsColour(LsCustoms.Vehicle, selectedColor.r, selectedColor.g, selectedColor.b)
                                LsCustoms.PriceNeonColor = 2500
                            end
                        })
    ...
    while main do Wait(1)
        RageUI.IsVisible(main, function()
            RageUI.Button('Kit Néons', nil, {RightLabel = '~g~15 000$ Unité~s~'}, true, {
                onSelected = function()
                    KeyboardUtils.use('Quantité', function(data)
                        TriggerServerEvent('sunny:caca:neon:paid', 'neonbox', data, 15000)
                    end)
                end
            })
            RageUI.Button('Controlleur Néons', nil, {RightLabel = '~g~25 000$ Unité~s~'}, true, {
                onSelected = function()
                    KeyboardUtils.use('Quantité', function(data)
                        TriggerServerEvent('sunny:caca:neon:paid', 'neoncontroller', data, 25000)
                    end)
                end
            })
        end)
