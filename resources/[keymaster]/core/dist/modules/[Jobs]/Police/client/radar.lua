...
-- local minSpeed = 0
local info = ""
local isRadarPlaced = false -- alphaaa
local Radar -- object
...
function radarSetSpeed(defaultText)
    local i = nil
    KeyboardUtils.use('Vitesse', function(data)
        local data = tonumber(data)

        if data == nil then i = 50 end

        i = data
    end)

    while not i do Wait(1) end
...
AddEventHandler('police:POLICE_radar', function (data)
    ...
    Citizen.CreateThread(function()
        ...
        while SunnyLSPD.Police.radar do
            Wait(0)
            ...
            if isRadarPlaced then
                ...
                if veh ~= nil then
                    ...
                    DrawRect(0.88, 0.97, 0.2, 0.03, 0, 0, 0, 85)
                    drawTxt(0.88, 0.97, 0.2, 0.03, 0.24, info, 255, 255, 255, 255)

                    DrawRect(0.88, 0.93, 0.2, 0.03, 0, 0, 0, 85)
