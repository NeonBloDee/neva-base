...
RegisterNetEvent('wais:removeTable:BodyCam', function(tableId)
    ...
    if id == "wais_listcammenu_open" then 
        lib.hideContext()
        ESX.ShowNotification(SunnyLSPD.Police.Camera.Lang["menu-updated"], 'error')
        listCams(lastMenu)
    end
end)
...
RegisterNetEvent('wais:AddOrRemove:DashCam', function()
    ...
    if carDistance <= 2 then
        ...
        ESX.ShowNotification(SunnyLSPD.Police.Camera.Lang["near-vehicles"], 'error')
    end
end)
...
if SunnyLSPD.Police.Camera.DevMode then
...
function CreateInstuctionScaleform(scaleform)
    ...
    InstructionButtonMessage(SunnyLSPD.Police.Camera.Lang["exit-cam"])
    PopScaleformMovieFunctionVoid()
...
function prepareCameraSelf(ped, activating)
    ...
    SetEntityCoords(ped, SunnyLSPD.Police.Camera.WatchCoords.x, SunnyLSPD.Police.Camera.WatchCoords.y, SunnyLSPD.Police.Camera.WatchCoords.z)
end
...
function openCamsMenu()
    ...
    local options = {
        {
            title = SunnyLSPD.Police.Camera.Lang["bodycam-list"],
            description = SunnyLSPD.Police.Camera.Lang["bodycam-list-desc"],
            icon = 'video',
            onSelect = function()
                listCams('bodycam')
            end,
        ...
        {
            title = SunnyLSPD.Police.Camera.Lang["dashcam-list"],
            description = SunnyLSPD.Police.Camera.Lang["dashcam-list-desc"],
            icon = 'clapperboard',
            onSelect = function()
                listCams('dashcam')
            end,
    ...
    TriggerEvent('wais:openMenu', 'wais_cammenu_open', SunnyLSPD.Police.Camera.Lang["main-menu-title"], options, nil, function(data)
...
function listCams(types)
    ...
    if types == "bodycam" then
        if not next(bodyCams) then
            ESX.ShowNotification(SunnyLSPD.Police.Camera.Lang["no-active-cam"], 'error')
            return
        end
        ...
        for k, v in pairs(bodyCams) do
            elements[#elements + 1] = {
                ...
                description = SunnyLSPD.Police.Camera.Lang["watch-bodycam"]:format(v.names),
                icon = 'video',
                onSelect = function()
    ...
    elseif types == "dashcam" then
        ...
        for k, v in pairs(carCams) do
            elements[#elements + 1] = {
                ...
                description = SunnyLSPD.Police.Camera.Lang["watch-dashcam"]:format(v.names),
                icon = 'video',
                onSelect = function()
...
CreateThread(function()
    while true do
        ...
        if ESXLoaded then
            ...
            if ESX.PlayerData.job.name == "police" or ESX.PlayerData.job.name == "bcso" and not inCam then
                local distance = #(coords - SunnyLSPD.Police.Camera.WatchCoords)
                if distance <= 4 then
                    sleep = 5
                    DrawText3D(SunnyLSPD.Police.Camera.WatchCoords.x, SunnyLSPD.Police.Camera.WatchCoords.y, SunnyLSPD.Police.Camera.WatchCoords.z, SunnyLSPD.Police.Camera.Lang["e-button-text"])
