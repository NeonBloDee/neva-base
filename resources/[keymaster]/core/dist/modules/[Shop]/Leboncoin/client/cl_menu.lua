...
OpenLeboncoin = function()
    ...
    TriggerServerEvent('Sunny:getInformations')
   ...
   -- mainlbc:SetSizeWidth(99)
    ...
    mainlbc.Closed = function()
        TriggerServerEvent("SunnyLpb:bucketlbcplayer", false)
    ...
    while mainlbc do 
        ...
        RageUI.IsVisible(mainlbc, function()
            ...
            RageUI.Button("> Déposer une annonce pour un véhicule", nil, {RightLabel = "→→→"}, true, {
                onSelected = function()
                    ESX.TriggerServerCallback("SunnyLpb:getplayerveh", function(Infoveh) 
                        VehPlayer = Infoveh
                    end)
                    Wait(10)
                        ...
                        if v.type == "obj" then 
                            RageUI.List("~r~> "..v.label.."", {"Supprimer", "Modifier le prix"}, Leboncoin.IndexListShowAnnonces, "Information sur l'annonce:\nPrix: ~r~"..v.price.." $~s~\nQuantité:~r~ x"..v.count.."", {}, true, {
                                ...
                                onSelected = function(Index)
                                    if Index == 1 then 
                                        TriggerServerEvent("SunnyLpb:updateannoncelbc", "delete", v.id)
                                        ESX.ShowNotification(SunnyConfigServ.GTACOLOR.."Slide~s~\nVous avez supprimé votre annonce")
                                    elseif Index == 2 then 
                                        local verif, newprice = CheckQuantity(ESX.GetTextInput("Indiquer le prix", "", "", 10))
                                        if verif then 
                                            TriggerServerEvent("SunnyLpb:updateannoncelbc", "price", v.id, newprice)
                    ...
                    for k, v in pairs(AnnoncesLeboncoin) do 
                        if v.type ~= "obj" then 
                            RageUI.List("~r~> "..GetLabelText(GetDisplayNameFromVehicleModel(v.props.model)).."", {"Supprimer", "Modifier le prix"}, Leboncoin.IndexListShowAnnonces, "Information sur l'annonce:\nPrix: ~r~"..v.price.." $~s~\nPlaque:~r~ "..v.plate.."", {}, true, {
                                ...
                                onSelected = function(Index)
                                    if Index == 1 then 
                                        TriggerServerEvent("SunnyLpb:updateannoncelbc", "delete", v.id, nil, "veh")
                                        ESX.ShowNotification(SunnyConfigServ.GTACOLOR.."Slide~s~\nVous avez supprimé votre annonce")
                                    elseif Index == 2 then 
                                        local verif, newprice = CheckQuantity(ESX.GetTextInput("Indiquer le prix", "", "", 10))
                                        if verif then 
        ...
        RageUI.IsVisible(objvente, function()
            ...
            if #ESX.PlayerData.inventory > 0 then 
                ...
                for k, v in pairs(ESX.PlayerData.inventory) do 
                    if SunnyConfigServ.BlackListItem[v.name] == nil then 
                        if v.count >= 0 then 
                        ...
                        RageUI.List("> "..GetLabelText(GetDisplayNameFromVehicleModel(v.props.model)).." ["..v.price.."$]", {"Acheter", "Visualiser"}, Leboncoin.IndexVehVente, v.name, {}, true, {
                            ...
                            onSelected = function(Index)
                                ...
                                elseif Index == 2 then 
                                    ...
                                    TriggerServerEvent("SunnyLpb:bucketlbcplayer", true)
                                    DeleteEntity(LastVeh)
                    ...
                    if v.type == "obj" then
                        ...
                        RageUI.Button("> "..v.label.." [x"..v.count.."]", nil, {RightLabel = "~r~"..v.price.." $"}, true, {
                            onSelected = function()
                                ...
                                if check then 
                                    TriggerServerEvent("SunnyLpb:buyobjlbc", v.id, v.UniqueID, quant)
                                    RageUI.GoBack()
                                end
                            end
                        })
        ...
        RageUI.IsVisible(valideachatveh, function()
            ...
            RageUI.Button("~r~Confirmer l'achat", nil, {RightLabel = "→→→"}, true, {
                onSelected = function()
                    TriggerServerEvent("SunnyLpb:achatveh", IdVeh, UniqueIDVeh)
        ...
        RageUI.IsVisible(venteobj, function()
            ...
            RageUI.Button("Valider la mise en vente", nil, {RightLabel = "→→→"}, true, {
                onSelected = function()
                    TriggerServerEvent("SunnyLpb:lbcventeitem", label, name, Leboncoin.CountLabel, Leboncoin.PriceLabel)
        ...
        RageUI.IsVisible(venteveh, function()
            ...
            RageUI.Button("Valider la mise en vente", nil, {RightLabel = "→→→"}, VerifPrixMec, {
                onSelected = function()
                    TriggerServerEvent("SunnyLpb:lbcventeveh", props, plate, Leboncoin.PricelabelVeh, labelveh, DescVeh, typee)
        ...
        RageUI.IsVisible(showventeargent, function()
            ...
            for k, v in pairs(VentePlayer) do 
                RageUI.Button("> Prix ~r~"..v.price, nil, {RightLabel = "Récupérer"}, true, {
                    onSelected = function()
                        TriggerServerEvent("SunnyLpb:recupmoneyvente", v.id, v.UniqueID, v.price)
                    end
                })
...
Citizen.CreateThread(function()
    ...
    Wait(1500)
    TriggerServerEvent("SunnyLpb:recieveannoncelbc")
    TriggerServerEvent('SunnyLpb:cballannonces')
end)

RegisterNetEvent("SunnyLpb:recieveclientsideannoncelbc")
AddEventHandler("SunnyLpb:recieveclientsideannoncelbc", function(Info)
    AnnoncesLeboncoin = Info   
end)

RegisterNetEvent("SunnyLpb:recievevehplayer")
AddEventHandler("SunnyLpb:recievevehplayer", function(veh)
    VehPlayer = veh
end)


RegisterNetEvent("SunnyLpb:cballannoncesafter")
AddEventHandler("SunnyLpb:cballannoncesafter", function(infos)
    AllAnnonces = infos 
end)

RegisterNetEvent("SunnyLpb:recieveventeplayer")
AddEventHandler("SunnyLpb:recieveventeplayer", function(infos)
    VentePlayer = infos 
end)
...
CreateThread(function()
            ...
            AddZones("LebonCoins", {
            Position = SunnyConfigServ.PosLeboncoin,
            Dist = 10,
            Public = true,
            Job = nil,
            Job2 = nil,
...
RegisterNetEvent('SunnyLpblegrosfdp:sendLBCInfo')
AddEventHandler('SunnyLpblegrosfdp:sendLBCInfo',function(LBC, LBCALL)
