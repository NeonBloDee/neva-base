CreateThread(function()
    ...
    RegisterNetEvent('sunny:boost:farm:paid', function(boost, boostTime, price)
        local source = source
        ...
        MySQL.Async.fetchAll("SELECT sunnycoins, totalCoins FROM users WHERE UniqueID = @UniqueID", {
            ['@UniqueID'] = xPlayer.UniqueID
        }, function(result)
            if #result == 0 then return end

            local newpoint = result[1].sunnycoins - price
            local newTotal = result[1].totalCoins + price

            MySQL.Async.execute('UPDATE users SET sunnycoins = @newpoint, totalcoins = @newTotal WHERE UniqueID = @UniqueID', {
                ['@newpoint'] = newpoint,
                ['@newTotal'] = newTotal,
                ['@UniqueID'] = xPlayer.UniqueID
            })
            ...
            MySQL.Async.execute([[
                ...
                TriggerClientEvent('sunny:boutique:boost:load', source, boost_farm.active[xPlayer.UniqueID])
            end)
    ...
    RegisterNetEvent('sunny:boutique:boost:request')
AddEventHandler('sunny:boutique:boost:request', function()
    local source = source
    while not boost_farm.loaded do Wait(1) end
                ...
                if v.boost > 0 then
                    ...
                    if v.time > 0 then
                        MySQL.Async.execute('UPDATE boost_farm SET time = @time, boost = @boost WHERE UniqueID = @UniqueID', {
                            ...
                            if xPlayer then
                                TriggerClientEvent('sunny:boutique:boost:load', xPlayer.source, { UniqueID = v.UniqueID, boost = 2, time = v.time, multiplication = 2 })
                            end
                        end)
                    else
                        MySQL.Async.execute('DELETE FROM boost_farm WHERE UniqueID = @UniqueID', {
                            ...
                            local xPlayer = ESX.GetPlayerFromIdentifier(v.UniqueID)
                            if xPlayer then
                                TriggerClientEvent('sunny:boutique:boost:load', xPlayer.source, { UniqueID = v.UniqueID, boost = 0, time = 0, multiplication = 0 })
                            end
                        end)
                    end
                end
