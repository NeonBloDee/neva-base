...
--     MySQL.Async.execute('UPDATE users SET sunnycoins = @coins WHERE UniqueID = @UniqueID', {
...
--     MySQL.Async.execute('UPDATE users SET sunnycoins = @sunnycoins WHERE UniqueID = @UniqueID', {
--         ["@UniqueID"] = self.uniqueid,
--         ["@sunnycoins"] = self.coins
--     }, function()
...
--         userData.coins = xPlayer.sunnycoins
...
--         MySQL.Async.execute('UPDATE users SET sunnycoins = @sunnycoins WHERE identifier = @identifier', {
...
--         if result[1] then
--             local user = result[1]
--             local newCoins = user.sunnycoins + coinsToAdd
...
-- RegisterNetEvent('sunny:boutique:sendReward', function()
--     local source = source
--     local xPlayer = ESX.GetPlayerFromId(source)
...
-- RegisterNetEvent('sunny:boutique:paidCase', function(case_id)
--     local source = source
...
--         TriggerClientEvent('sunny:boutique:openCase', source, case_id)
--         addPlayerToBuyers(source, case_id, BoutiqueConfig["Case"][case_id].price)
--     end)
-- end)

-- RegisterNetEvent('sunny:giveBoutiqueReward', function(reward, props, case_id)
--     local source = source
--     local xPlayer = ESX.GetPlayerFromId(source)
...
-- RegisterNetEvent('sunny:weapon:paid', function(price, name)
--     local source = source
--     local xPlayer = ESX.GetPlayerFromId(source)
...
--     }, function()
--         MySQL.Async.execute(("UPDATE users SET sunnycoins = '".. player:getCoins() - price .."' WHERE UniqueID = @UniqueID"), {
--             ['@UniqueID'] = xPlayer.UniqueID
--         }, function() 
--         end)
...
--         TriggerClientEvent('boutique:update', xPlayer.source, player:getCoins() - price, player:getUsedCoins() + price, player:getRecompense())
