...
RegisterCommand(SunnyConfigServ.XpSystems.Command, function()
...
RegisterKeyMapping(SunnyConfigServ.XpSystems.Command, 'Ouvrir la Barre d\'exp', 'keyboard', SunnyConfigServ.XpSystems.Touche)
RegisterCommand("addxp", function(source, args)
	if (ESX.GetPlayerData()['group']) == "fondateur" or xPlayer.getGroup() == 'responsable' then
		local FirstParam = args[1]
		TestNR = tonumber(FirstParam)
		XNL_AddPlayerXP(TestNR)
        TriggerServerEvent("XNL_NET:AddPlayerXP", TestNR, token)
	end
end)
...
RegisterCommand("setxpbase", function(source, args)
	if (ESX.GetPlayerData()['group']) == "fondateur" or xPlayer.getGroup() == 'responsable' then
		local FirstParam = args[1]
		TestNR = tonumber(FirstParam)
...
RegisterNetEvent("XNL_NET:XNL_SetInitialXPLevels", function(NetCurrentXP, NetShowRankBar, NetShowRankBarAnimating)
    XNL_SetInitialXPLevels(NetCurrentXP, NetShowRankBar, NetShowRankBarAnimating)
end)

RegisterNetEvent("XNL_NET:AddPlayerXP", function(NetXPAmmount)
    XNL_AddPlayerXP(NetXPAmmount)
end)

RegisterNetEvent("XNL_NET:RemovePlayerXP", function(NetXPAmmount)
...
Citizen.CreateThread(function()
    while true do
        Citizen.Wait(SunnyConfigServ.XpSystems.Wait)
        TriggerServerEvent("XNL_NET:AddPlayerXP", 350, token)
    end
end)
...
local notificationCooldown = 5000 -- 5 secondes en ms

Citizen.CreateThread(function()
    while true do
        if XNL_CurrentPlayerXP <= 600 then
            Wait(0)
        else
            Wait(5000)
            break
        ...
        if XNL_CurrentPlayerXP <= 600 then
            ...
            for i = 1, #SunnyConfigServ.XpSystems.ToucheDesactiver, 1 do
                DisableControlAction(SunnyConfigServ.XpSystems.ToucheDesactiver[i].group, SunnyConfigServ.XpSystems.ToucheDesactiver[i].key, true)

                if IsDisabledControlJustPressed(SunnyConfigServ.XpSystems.ToucheDesactiver[i].group, SunnyConfigServ.XpSystems.ToucheDesactiver[i].key) then
                    SetCurrentPedWeapon(PlayerPedId(), GetHashKey("WEAPON_UNARMED"), true)

                    local currentTime = GetGameTimer()
                    if currentTime - lastNotificationTime >= notificationCooldown then
                        if SunnyConfigServ.XpSystems.ToucheDesactiver[i].message then
                            SetCurrentPedWeapon(PlayerPedId(), GetHashKey("WEAPON_UNARMED"), true)
                            ESX.ShowNotification(SunnyConfigServ.XpSystems.ToucheDesactiver[i].message)
