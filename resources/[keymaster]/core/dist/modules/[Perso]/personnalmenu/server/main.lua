RegisterNetEvent('sunny:entreprise:Boss_recruterplayer', function(target)
    local source = source
...
ESX.RegisterServerCallback('Sunny:vip:getPlates', function(source, cb)
    local xPlayer = ESX.GetPlayerFromId(source)
    local plates = {}
...
ESX.RegisterServerCallback('sunny:getNumberOfSocieties', function(source, cb)
...
RegisterNetEvent('Sunny:vip:changePlate')
AddEventHandler('Sunny:vip:changePlate', function(vehicleNetId, newPlate)
    local xPlayer = ESX.GetPlayerFromId(source)
    local vehicle = NetworkGetEntityFromNetworkId(vehicleNetId)

    if DoesEntityExist(vehicle) then
        local currentPlate = GetVehicleNumberPlateText(vehicle)
        print("Véhicule trouvé. Plaque actuelle:", currentPlate, "Nouvelle plaque:", newPlate)
        ...
        MySQL.Async.execute('UPDATE owned_vehicles SET plate = @newPlate WHERE plate = @oldPlate', {
            ...
            if rowsChanged > 0 then
                ...
                TriggerClientEvent('Sunny:vip:updateVehiclePlate', -1, vehicleNetId, newPlate)
            else
                TriggerClientEvent('esx:showNotification', source, '~r~Erreur : Impossible de mettre à jour la plaque dans la base de données.')
            end
        end)
    else
        print("Véhicule introuvable. vehicleNetId:", vehicleNetId)
...
RegisterNetEvent('sunny:entreprise:Boss_virerplayer', function(target)
    local source = source
    local xPlayer = ESX.GetPlayerFromId(source)
    local targetPlayer = ESX.GetPlayerFromId(target)
...
AddEventHandler('boss:Boss_recruterplayer2', function(target, grade2)
	...
	if sourceJob2.grade_name == 'boss' then
		...
		targetXPlayer.setJob2(sourceJob2.name, 1)
        TriggerClientEvent('esx:showNotification', sourceXPlayer.source, "Vous avez recruté "..targetXPlayer.name)
		TriggerClientEvent('esx:showNotification', target, "Vous avez été embauché par "..sourceXPlayer.name)
	end
end)
...
RegisterServerEvent("Sunny:job:leave")
AddEventHandler("Sunny:job:leave", function()
    local source = source 
	local targetXPlayer = ESX.GetPlayerFromId(source)
	targetXPlayer.setJob('unemployed', 0)
end)
RegisterServerEvent("Sunny:job2:leave")
AddEventHandler("Sunny:job2:leave", function()
    local source = source 
	local targetXPlayer = ESX.GetPlayerFromId(source)
...
RegisterNetEvent('sunny:entreprise:Boss_destituerplayer', function(target)
    local source = source
    local xPlayer = ESX.GetPlayerFromId(source)
    local targetPlayer = ESX.GetPlayerFromId(target)
...
RegisterNetEvent('sunny:personalmenu:getpropspropss', function()
    local source = source
    local xPlayer = ESX.GetPlayerFromId(source)
    ...
    MySQL.Async.fetchAll('SELECT * FROM players_props WHERE UniqueID = @UniqueID', {
        ...
        TriggerClientEvent('sunny:recieveProps', source, table[xPlayer.UniqueID])
    end)
end)

RegisterNetEvent('sunny:bill:pay', function(v)
    local source = source
    local xPlayer = ESX.GetPlayerFromId(source)
