...
RegisterCommand('+sper', function()
    if not canRagdoll then return end
    if not DoesEntityExist(PlayerPedId()) then return end
    if not IsPedInAnyVehicle(PlayerPedId(), false) and not IsPedSwimming(PlayerPedId()) and not IsPedShooting(PlayerPedId()) and not IsPedClimbing(PlayerPedId()) and not IsPedCuffed(plyPed) and not IsPedDiving(PlayerPedId()) and not IsPedFalling(PlayerPedId()) and not IsPedJumpingOutOfVehicle(PlayerPedId()) and not IsPedUsingAnyScenario(PlayerPedId()) and not IsPedInParachuteFreeFall(PlayerPedId()) then
        IsRagdollPut = not IsRagdollPut
        if IsRagdollPut then
            Citizen.CreateThread(function()
...
RegisterNetEvent('Sunny:vip:updateVehiclePlate')
AddEventHandler('Sunny:vip:updateVehiclePlate', function(vehicleNetId, newPlate)
    local vehicle = NetworkGetEntityFromNetworkId(vehicleNetId)

    if DoesEntityExist(vehicle) then
        SetVehicleNumberPlateText(vehicle, newPlate)
        ESX.ShowNotification("~g~Plaque mise à jour visuellement en : " .. newPlate)
        print("Plaque mise à jour visuellement :", newPlate)
    else
        print("Véhicule introuvable pour mise à jour visuelle.")
    end
end)
...
RegisterCommand('noir', function()
    ...
    if noir then 
        ...
        TriggerEvent('sunny:zzhud:update', false)
    end
    while noir do
    ...
    TriggerEvent('sunny:zzhud:update', true)
...
Citizen.CreateThread(function()
    ...
    local value = GetResourceKvpString(("%s:weaponTint"):format(SunnyConfigServ.ServerName))
...
function setWeaponTint(weaponTint)
    SetResourceKvp(("%s:weaponTint"):format(SunnyConfigServ.ServerName), weaponTint)
    SetPedWeaponTintIndex(Player.playerPed, Player.weapon, tonumber(weaponTint))
    return value
end

function getWeaponTint(name)
    return tonumber(GetResourceKvpString(("%s:weaponTint"):format(SunnyConfigServ.ServerName)))
end

AddEventHandler('sunny:utils:changeweapon', function(weaponHash)
...
Citizen.CreateThread(function()
    ...
    for k, v in pairs(Config.WeaponsBinds) do
        ...
        if value == nil or value2 == nil then
            TriggerEvent("sunny:inventory:sendSlots2", k, {label = v.weaponLabel, name = v.weapon})
            setWeaponKeybind(k, v.weapon, v.weaponLabel)
        else
            -- print('Loading saved weapon for slot', k)
            TriggerEvent("sunny:inventory:sendSlots2", k, {label = value2, name = value})
...
function setWeaponKeybind(name, weaponHash, weaponLabel)
    ...
    --TriggerEvent("sunny:inventory:sendSlots2", name, weaponHash)
