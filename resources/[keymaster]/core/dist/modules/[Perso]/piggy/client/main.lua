...
RegisterCommand('porter', function(source, args)
    ...
    if not GetSafeZone() then
        ...
        if closestDistance ~= -1 and closestDistance <= 3 then
            ...
            -- Lance l'événement serveur pour synchroniser les animations
            ...
            -- Démarre une surveillance constante pour arrêter le portage
            monitorStopConditions(target)
        else
...
-- Fonction pour arrêter le portage
function stopLift(ped)
    print("Arrêt du portage.")
    ClearPedSecondaryTask(ped) -- Arrête l'animation secondaire
    DetachEntity(ped, true, false) -- Détache le joueur porté
    piggyBackInProgress = false -- Réinitialise l'état du portage

    -- Informer le serveur pour arrêter l'animation de l'autre joueur
    local closestPlayer, closestDistance = ESX.Game.GetClosestPlayer()
    if closestDistance ~= -1 and closestDistance <= 3 then
        local target = GetPlayerServerId(closestPlayer)
...
-- Surveillance constante pour arrêter le portage
function monitorStopConditions(target)
    Citizen.CreateThread(function()
        while piggyBackInProgress do
            -- Affiche une notification d'aide
            ESX.ShowHelpNotification('Appuyez sur X ou /porter pour lâcher le joueur.')

            -- Vérification d'autres conditions pour arrêter
            if IsControlJustReleased(0, 73) or not piggyBackInProgress then -- INPUT_DROP (X) ou commande relancée
                piggyBackInProgress = false
                stopLift(Player.playerPed)
                ESX.ShowNotification('Vous avez lâché le joueur.')
                break
            end		
...
-- Gestion côté serveur pour synchroniser les arrêts
RegisterNetEvent('cmg2_animations:stop')
AddEventHandler('cmg2_animations:stop', function(target)
    TriggerClientEvent('cmg2_animations:stopOnClient', target)
end)

-- Gestion côté client pour arrêter les animations
RegisterNetEvent('cmg2_animations:stopOnClient')
AddEventHandler('cmg2_animations:stopOnClient', function()
...
RegisterCommand('tacle', function()
    ...
    if not SunnyConfigServ.Plaquage.whielistJobs[ESX.PlayerData.job.name] then
        return
    end
...
RegisterKeyMapping(SunnyConfigServ.Plaquage.Command, 'Plaquer un Joueur', 'keyboard', SunnyConfigServ.Plaquage.Touche)