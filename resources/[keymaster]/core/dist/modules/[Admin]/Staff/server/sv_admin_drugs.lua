...
CreateThread(function()
    ...
    RegisterNetEvent('sunny:drugs:load', function()
        local source = source

        TriggerClientEvent('sunny:drugs:load', source, drugs_circuits)
    end)

    RegisterNetEvent('sunny:drugs:create', function(data)
        local source = source
        ...
        MySQL.Async.execute('INSERT INTO drugs_circuits (name, label, recolte, traitement, animtype, animdict, anim, animtime, marker, props, name_pooch, label_pooch, animtype_t, animdict_t, anim_t, animtime_t, marker_t, props_t) VALUES (@name, @label, @recolte, @traitement, @animtype, @animdict, @anim, @animtime, @marker, @props, @name_pooch, @label_pooch, @animtype_t, @animdict_t, @anim_t, @animtime_t, @marker_t, @props_t)', {
            ...
            TriggerClientEvent('sunny:drugs:load', -1, drugs_circuits)

            TriggerClientEvent('sunny:drugs:refresh', -1, data.name)
        end)
    end)

    RegisterNetEvent('sunny:admin:drugs:update', function(drugsName, data)
        local xPlayer = ESX.GetPlayerFromId(source)
	    if xPlayer.getGroup() == 'user' then return end
    ...
    RegisterNetEvent('sunny:labs:fetchAll', function()
        local source = source
        TriggerClientEvent('sunny:labs:recieve', source, drugs_laboratories)

        TriggerClientEvent('sunny:admin:drugs:sendSell', source, drugs_sell)
    end)

    RegisterNetEvent('sunny:admin:labo:create', function(data)
        local xPlayer = ESX.GetPlayerFromId(source)
	if xPlayer.getGroup() == 'user' then return end
        MySQL.insert("INSERT INTO illegal_laboratory (name, type, interior, owner, pos) VALUES (?,?,?,?,?)", {
            ...
            TriggerClientEvent('sunny:labs:recieve', -1, drugs_laboratories)
        end)
    end)

    RegisterNetEvent('sunny:drugs:sell:addPos', function(pos, message)
        local xPlayer = ESX.GetPlayerFromId(source)
	if xPlayer.getGroup() == 'user' then return end
        ...
        TriggerClientEvent('sunny:admin:drugs:sendSell', -1, drugs_sell)
    end)

    RegisterNetEvent('sunny:drugs:sell:delete', function(id)
        local xPlayer = ESX.GetPlayerFromId(source)
	if xPlayer.getGroup() == 'user' then return end
