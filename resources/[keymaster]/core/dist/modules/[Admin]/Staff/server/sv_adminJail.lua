CreateThread(function()
    ...
    AddEventHandler('esx:playerLoaded', function(source, player)
        ...
        for k,v in pairs(adminManagement.jails) do
            if v.UniqueID == xPlayer.UniqueID or tonumber(v.UniqueID) == tonumber(xPlayer.UniqueID) or tostring(v.UniqueID) == tostring(xPlayer.UniqueID) then
                TriggerClientEvent('sunny:admin:jail:setPlayerInJail', source, xPlayer.UniqueID, adminManagement.jails[xPlayer.UniqueID])
    ...
    RegisterNetEvent('sunny:admin:jail:remove', function(UniqueID, data)
        local source = source
        adminManagement.jails[tonumber(UniqueID)] = nil

        TriggerClientEvent('sunny:admin:jail:remove', source, UniqueID)
    ...
    RegisterCommand('jail', function(source, args)
        ...
        MySQL.Async.execute('DELETE FROM player_jails WHERE UniqueID = @UniqueID', {
            ...
            MySQL.Async.fetchAll('SELECT * FROM player_jails WHERE UniqueID = @UniqueID', {
                ['@UniqueID'] = args[1]
            }, function(result)
                if #result > 0 then
                    if source ~= 0 then
                        TriggerClientEvent('esx:showNotification', source, 'Ce joueur est déjà en jail dans la base de données')
                    end
                    return
                end
                ...
                if not player then
                    ...
                    MySQL.Async.execute('INSERT INTO player_jails (UniqueID, time, staff, reason) VALUES (?,?,?,?)', {
                        ...
                        TriggerClientEvent('sunny:admin:jail:setPlayerInJail', player.id, player.UniqueID, adminManagement.jails[tonumber(args[1])])
                    end)
                end
            end)
        end)
    ...
    RegisterCommand('unjail', function(source, args)
        ...
        if player then
            TriggerClientEvent('sunny:admin:jail:remove', player.id, argsssss)
        end
