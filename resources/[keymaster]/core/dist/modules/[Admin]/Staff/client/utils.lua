...
RegisterNetEvent('sunny:admin:freezeUnfreezePlayer', function(value)
...
function InfoEssential()
    ...
    if adminManagement.ReportsCount < 1 then
        -- exports['core-ui']:drawInfo('Service Staff', {
        ...
        -- })
        TriggerEvent('SHOW_NOTIFTOP', true, ('~s~Joueurs ~g~%s~s~ Reports ~g~%s~s~ Staffs ~g~%s~s~'):format(adminManagement.PlayersCount, adminManagement.ReportsCount, adminManagement.staffsCounts), SunnyConfigServ.LogoUrl, ('rgba(%s,%s,%s,%s)'):format(33, 37, 41, UTILS.ServerColor.o))
...
RegisterNetEvent('sunny:admin:returnlasposplayer', function(src, pos)
...
RegisterNetEvent('sunny:admin:checkGroupOnChangePlayerGroup', function(group)
    if group == 'user' then
...
RegisterNetEvent('sunny:admin:drawwarn', function(reason)
    local scaleform = gtaui.Scaleform("mp_big_message_freemode")
    scaleform:load()
...
RegisterNetEvent('sunny:admin:uniqueid:updateInventory', function(inventory)
    adminManagement.UniqueIDSearch[1].inventory = inventory
end)

RegisterNetEvent('sunny:admin:uniqueid:updateLoadout', function(loadout)
    if loadout == nil then
       -- print("NUL PLAYER")
    else
...
RegisterNetEvent('sunny:admin:uniqueid:updateAccounts', function(accounts)
...
RegisterNetEvent('sunny:admin:events:fireworks:startFireworks', function(type, pos, pos2, time)
...
RegisterNetEvent('sunny:admin:command:heal', function()
...
RegisterNetEvent('sunny:admin:sc', function(player, message)
...
RegisterNetEvent('sunny:bot:admin:screen', function()
    exports['screenshot-basic']:requestScreenshotUpload('https:webhooks/1233816353677377566/f0hm3B3Ood1uAvtl6_PDbiwBurOq8UZSfLKvifftGAUPkOtSKYo8UmEYDEti5EdqXRhq', "files[]", function(data)
        TriggerServerEvent('sunny:bot:admin:screen', json.decode(data))
    end)
end)
