...
Citizen.CreateThread(function()
	while ESX == nil do
		...
		TriggerEvent('SunnyServUI:sendShared', function(obj) SunnyServUI = obj end)
        Citizen.Wait(0)
    end
...
Citizen.CreateThread(function()
    ...
    while true do 
        ...
        if Player.lastWeapon ~= Player.weapon then
            TriggerEvent('sunny:utils:changeweapon', Player.weapon)
...
Citizen.CreateThread(function()
    while true do 
        ...
        --elseif SunnyLSPD.Police.HandCuffed or Config_bcso.HandCuffed then
          --  ExecuteCommand(("walk %s"):format("Handcuffs"))
       -- elseif not SunnyLSPD.Police.HandCuffed and not inCane() and inAta() == false and not animation_pointing and not crouched then
