...
RegisterKeyMapping('-OpenChat', 'Ouvrir le chat textuel', 'keyboard', 'T')
RegisterCommand("-OpenChat", function()
    if exports["ui"] and exports["ui"].isInInterface and exports["ui"]:isInInterface() then return end
...
RegisterNUICallback('chatResult', function(data, cb)
    ...
    if not data.canceled and data.message then
        if string.sub(data.message, 1, 1) == '/' then
            ...
            local id = PlayerId()
            TriggerServerEvent('_chat:messageEntered', GetPlayerName(id), {0, 0, 255}, data.message)
        end
    end
