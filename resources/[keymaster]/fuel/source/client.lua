    ...
    while true do
        ...
        if wastingFuel then
            ...
            while wastingFuel do
                ...
                DrawInstructionBarNotification(nozzleLocation.x, nozzleLocation.y, nozzleLocation.z, ("Coût: ~r~$%s~s~"):format(cost))
    ...
    while true do
        ...
        if pump then
            ...
            if not holdingNozzle and not nozzleInVehicle and not nozzleDropped then
                DrawInstructionBarNotification(pump.x, pump.y, pump.z + 1.2, "~g~[E]~s~ Prendre le pistolet d'essence")
            ...
            elseif holdingNozzle and not nearTank and pumpHandle == usedPump then
                -- Montre combien d'essence au dessus du veh
                fuel = GetFuel(vehicleFueling)
                vehpos = GetEntityCoords(vehicleFueling)

                DrawInstructionBarNotification(vehpos.x, vehpos.y, vehpos.z + 1.2, ("Essence: ~y~%s%%~s~"):format(math.floor(fuel)))

                DrawInstructionBarNotification(pump.x, pump.y, pump.z + 1.2, "~r~[E]~s~ Déposer le pistolet d'essence")
                if IsControlJustPressed(0, 51) then
                    LoadAnimDict("anim@am_hold_up@male")
                    TaskPlayAnim(ped, "anim@am_hold_up@male", "shoplift_high", 2.0, 8.0, -1, 50, 0, 0, 0, 0)
                    Wait(300)
        ...
        if holdingNozzle or nozzleInVehicle or nozzleDropped then
            ...
            if nozzle and pumpCoords then
                ...
                if nozzleDropped and #(nozzleLocation - pedCoords) < 1.5 then
                    DrawInstructionBarNotification(nozzleLocation.x, nozzleLocation.y, nozzleLocation.z, "~g~[E]~s~ Prendre le tuyau")
                    if IsControlJustPressed(0, 51) then
            ...
            if veh then
                ...
                if tankPosition and #(pedCoords - tankPosition) < 1.2 then
                    if not nozzleInVehicle and holdingNozzle then
                        ...
                        DrawInstructionBarNotification(vehpos.x, vehpos.y, vehpos.z + 1.2, ("Essence: ~y~%s%%~s~"):format(math.floor(fuel)))
                        DrawInstructionBarNotification(tankPosition.x + textModifiedPosition.x, tankPosition.y + textModifiedPosition.y, tankPosition.z + zPos + textModifiedPosition.z, "~g~[E]~s~ Mettre le pistolet d'essence")
                        if IsControlJustPressed(0, 51) then
                            LoadAnimDict("timetable@gardener@filling_can")
                            TaskPlayAnim(ped, "timetable@gardener@filling_can", "gar_ig_5_filling_can", 2.0, 8.0, -1, 50, 0, 0, 0, 0)
                            Wait(300)
                    ...
                    elseif nozzleInVehicle then
                        ...
                        DrawInstructionBarNotification(vehpos.x, vehpos.y, vehpos.z + 1.2, ("Remplissage en cours: ~y~%s%%~s~"):format(math.floor(fuel)))
                        DrawInstructionBarNotification(tankPosition.x + textModifiedPosition.x, tankPosition.y + textModifiedPosition.y, tankPosition.z + zPos + textModifiedPosition.z, "~g~[E]~s~ Reprendre le pistolet d'essence")
                        if IsControlJustPressed(0, 51) then
        ...
        if GetSelectedPedWeapon(ped) == 883325847 and not holdingNozzle and not nozzleInVehicle then
            ...
            if veh then
                ...
                if tankPosition and #(pedCoords - tankPosition) < distance then
                    local fuel = GetFuel(veh)
                    DrawInstructionBarNotification(tankPosition.x, tankPosition.y, tankPosition.z + zPos, ("~y~%s%%~s~ ~g~[E]~s~ Remplir le réservoir"):format(math.floor(fuel)))
