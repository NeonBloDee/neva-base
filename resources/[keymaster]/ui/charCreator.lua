...
Citizen.CreateThread(function()
    while true do
        ...
        if playerData ~= nil and playerData ~= nil then
        end
    end
end)
...
creatorCoords = vec3(-811.801086, 175.067963, 76.745377-0.98)
creatorHeading = 111.42098236084

AddEventHandler("ui:charCreator:open", function()
    ...
    SetEntityCoords(playerPed, creatorCoords)
    SetEntityHeading(playerPed, creatorHeading)
    ...
    Citizen.CreateThread(function()
        while isInCreator do 
            ...
            if #(coords-creatorCoords) > 10 then
                SetEntityCoords(PlayerPedId(), creatorCoords)
                SetEntityHeading(PlayerPedId(), creatorHeading)
            end
...
RegisterNUICallback("creator:rotateCam", function(data)
...
RegisterNUICallback("creator:setParent", function(data)
    zoomOffset = 0.6
    camOffset = 0.65
    if data.name == "Parent1" then
        TriggerEvent("skinchanger:change", "dad", data.id)
    elseif data.name == "Parent2" then
        TriggerEvent("skinchanger:change", "mom", data.id)
    end
end)

RegisterNUICallback("creator:setDNAValue", function(data)
    if not isInCreator then return end
...
RegisterNUICallback("creator:setSex", function(data)
    TriggerEvent('skinchanger:change', "sex", tonumber(data.value))
end)

RegisterNUICallback("creator:saveCharacter", function(data)
    DisplayRadar(true)
    ...
    TriggerEvent('skinchanger:getSkin', function(skin)
        ...
        TriggerServerEvent("ui:charCreator:finish", data, skin)
    end)
end)

RegisterNUICallback("creator:resetCharacter", function(data)
    TriggerEvent("skinchanger:resetSkin")
end)

RegisterNUICallback("creator:saveCharacterFailed", function()
    SendNUIMessage({
        type = "inventory:sendMessage",
        data = {
            message = {text = "Vous n'avez pas correctement crée votre identité", icon = icon}
        }
    })
end)