    ...
    ["bproof_1"] = {
        ...
        resetValue = {
            ["bproof_2"] = {
                default = 0, 
                maxValue = function()
                    return GetNumberOfPedTextureVariations(PlayerPedId(), 11, exports.sunny:GetPlayerSkinData('bproof_1') or 1 - 1)
                end
            }
        },
    },
...
RegisterNUICallback("shop:setClotheValue", function(data)
    ...
    if data.name == "bags_1" then
        local bagData = exports["core"]:getBagById(GetEntityModel(PlayerPedId()) == GetHashKey("mp_m_freemode_01"), tonumber(data.value))
        print(bagData, json.encode(bagData), bagData.weight, bagData.weapon)
        exports['core-ui']:drawInfo(("Information Sac"):format(), {
...
RegisterNUICallback("shop:buyAccessories", function(clotheData)
    print('buyyyyy ta mere la pute', table.unpack(clotheData))
        ...
        local data = {data = {{name = clotheData.name, number = exports.sunny:GetPlayerSkinData(clotheData.name)}, {name = creatorData[clotheData.name].secondeName, number = exports.sunny:GetPlayerSkinData(creatorData[clotheData.name].secondeName)}}}
        data.label = label
        ESX.TriggerServerCallback('sunny:clotheshop:haveMoneySave', function(valid)
            ...
            if (valid) then 
                ...
                TriggerEvent("skinchanger:getSkin", function(skin)
                    ...
                    TriggerServerEvent("sunny:paidaccesoires", clotheData.name, label, {
