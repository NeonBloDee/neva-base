...
wd.debug = function(str)
    if cfg?.debug or not cfg then
        if IsDuplicityVersion() then
            print('[^1wendigo^0] - [^1Debug^0]: ' .. str)
        else
            print('[wendigo] - [Debug]: ' .. str)
        end
    end
end
...
if not IsDuplicityVersion() then --client
    wd.notification = function(type, str, length)
    ...
    RegisterNetEvent(GetCurrentResourceName() .. ':client:notification', wd.notification)

    wd.serverCallbacks = {}

    wd.triggerCallback = function(name, cb, ...)
        wd.serverCallbacks[name] = cb
    ...
    RegisterNetEvent(GetCurrentResourceName() .. ':client:triggerCallback', function(name, ...)
        if wd.serverCallbacks[name] then
            wd.serverCallbacks[name](...)
            wd.serverCallbacks[name] = nil
        end
    end)

    wd.registerKeyMap = function(data, cb, cb2)
        RegisterCommand('+wd_' .. data.command, function()
            local response = true
        ...
        if data.key:match('mouse') or data.key:match('iom') then
            RegisterKeyMapping('+wd_' .. data.command, data.description, 'mouse_button', data.key:lower())
        else
            RegisterKeyMapping('+wd_' .. data.command, data.description, 'keyboard', data.key:lower())
        end

        Wait(500)
        TriggerEvent('chat:removeSuggestion', ('/+wd_%s'):format(data.command))
        TriggerEvent('chat:removeSuggestion', ('/-wd_%s'):format(data.command))
    end
    ...
    wd.getPlayersKeyPreference = function(command, cb)
        local key = GetControlInstructionalButton(0, joaat('+wd_' .. command) | 0x80000000, true):sub(3)
        if specialChars[key] then key = specialChars[key] end
        if cb then cb(key) else return key end
    end

    wd.drawText3D = function(x, y, z, str, length, r, g, b, a)
    ...
    wd.addBlip = function(coords, sprite, scale, color, str, cb)
    ...
    wd.drawSubtitle = function(str, time)
        BeginTextCommandPrint('STRING')
        AddTextComponentSubstringPlayerName(str)
        EndTextCommandPrint(time or 4000, 1)
    end

    wd.drawBusySpinner = function(str)
        BeginTextCommandBusyspinnerOn('STRING')
        AddTextComponentSubstringPlayerName(str)
        EndTextCommandBusyspinnerOn(3)
    end

    wd.loadAnimDict = function(dict, cb)
    ...
    wd.loadPtfxAsset = function(asset, cb)
    ...
    wd.loadAnimSet = function(set, cb)
    ...
    wd.requestModel = function(model, cb)
    ...
    wd.spawnObject = function(model, coords, isLocal, cb)
        model = type(model) == 'number' and model or joaat(model)

        wd.requestModel(model, function()
    ...
    wd.spawnPed = function(model, coords, heading, isLocal, cb)
        model = type(model) == 'number' and model or joaat(model)
    ...
    wd.spawnVehicle = function(model, coords, heading, isLocal, cb)
        model = type(model) == 'number' and model or joaat(model)
    ...
    wd.deleteObject = function(object, cb)
        SetEntityAsMissionEntity(object, false, true)
        DeleteObject(object)
        if cb then cb() else return end
    end

    wd.deleteVehicle = function(vehicle, cb)
        SetEntityAsMissionEntity(vehicle, false, true)
        DeleteVehicle(vehicle)
        if cb then cb() else return end
    end

    wd.deletePed = function(ped, cb)
        SetEntityAsMissionEntity(ped, false, true)
        DeletePed(ped)
        if cb then cb() else return end
    end

    wd.getVehicles = function()
        return GetGamePool('CVehicle')
    end

    wd.getObjects = function()
        return GetGamePool('CObject')
    end

    wd.getPlayers = function()
        return GetActivePlayers()
    end

    wd.getClosestPed = function(coords)
    ...
    wd.getClosestPeds = function(coords, maxDistance)
        local ped = PlayerPedId()
        local peds = GetGamePool('CPed')
        coords = coords and (type(coords) == 'table' and vec3(coords.x, coords.y, coords.z) or coords) or GetEntityCoords(ped)
    ...
    wd.getClosestPlayer = function(coords)
        local ped = PlayerPedId()
        coords = coords and (type(coords) == 'table' and vec3(coords.x, coords.y, coords.z) or coords) or GetEntityCoords(ped)
        local closestPlayers = wd.getPlayersFromCoords(coords)
    ...
    wd.getPlayersFromCoords = function(coords, maxDistance)
        local players = wd.getPlayers()
        local ped = PlayerPedId()
        coords = coords and (type(coords) == 'table' and vec3(coords.x, coords.y, coords.z) or coords) or GetEntityCoords(ped)
        local maxDistance = maxDistance or 5
        local closePlayers = {}
    ...
    wd.getClosestVehicle = function(coords)
        local ped = PlayerPedId()
    ...
    wd.getClosestVehicles = function(coords, maxDistance)
        local ped = PlayerPedId()
        local vehicles = GetGamePool('CVehicle')
        coords = coords and (type(coords) == 'table' and vec3(coords.x, coords.y, coords.z) or coords) or GetEntityCoords(ped)
        local maxDistance = maxDistance or 5
    ...
    wd.getClosestObject = function(coords)
        local ped = PlayerPedId()
    ...
    wd.getClosestObjects = function(coords, maxDistance)
        local ped = PlayerPedId()
        local objects = GetGamePool('CObject')
        coords = coords and (type(coords) == 'table' and vec3(coords.x, coords.y, coords.z) or coords) or GetEntityCoords(ped)
        local maxDistance = maxDistance or 5
    ...
    local duis = {}
    wd.createDui = function(url, width, height, cb)
        width = width or 512
    ...
    wd.changeDuiUrl = function(id, url)
        if not duis[id] then
    ...
    wd.raycast = function(origin, target, options, ignoreEntity, radius)
    ...
    wd.getEntityPlayerIsLookingAt = function(maxDistance, radius, flag, ignore)
        ...
        local _, hit, targetCoords, _, targetEntity = wd.raycast(originCoords, forwardCoords, flag or 4294967295, ignore, radius or 0.2)
    ...
    wd.getEntityInFrontOfEntity = function(entity, maxDistance, radius, flag)
        local maxDistance = maxDistance or 3.0
        ...
        local _, hit, _, _, targetEntity = wd.raycast(originCoords, targetCoords, flag or 4294967295, entity, radius or 0.2)
        return targetEntity
    end

    wd.getVehicleProperties = function(vehicle)
        if DoesEntityExist(vehicle) then
    ...
    wd.setVehicleProperties = function(vehicle, props)
        if DoesEntityExist(vehicle) then
    ...
    wd.registerCallback = function(name, cb)
        wd.serverCallbacks[name] = cb
    end

    wd.triggerCallback = function(name, src, cb, ...)
        if wd.serverCallbacks[name] then
            wd.serverCallbacks[name](src, cb, ...)
        else
            wd.debug('This callback(^2' .. name .. '^0) is not registered!')
        end
    end
    ...
    RegisterNetEvent(GetCurrentResourceName() .. ':server:triggerCallback', function(name, ...)
        ...
        wd.triggerCallback(name, src, function(...)
    ...
    if loadSqlFuncs then
        ...
        if GetResourceState('oxmysql') == 'started' then
            CreateThread(function()
                ...
                wd.sql.sync.query = MySQL.query.await
                wd.sql.sync.insert = MySQL.insert.await
                wd.sql.sync.update = MySQL.update.await
                -- wd.sql.sync.single = MySQL.single.await
                wd.sql.sync.scalar = MySQL.scalar.await

                wd.sql.async.query = MySQL.query
                wd.sql.async.insert = MySQL.insert
                wd.sql.async.update = MySQL.update
                -- wd.sql.async.single = MySQL.single
                wd.sql.async.scalar = MySQL.scalar
            end)
        elseif GetResourceState('mysql-async') == 'started' then
            CreateThread(function()
                ...
                MySQL.ready(function()
                    wd.sql.sync.query = MySQL.Sync.fetchAll
                    wd.sql.sync.insert = MySQL.Sync.insert
                    wd.sql.sync.update = MySQL.Sync.execute
                    -- wd.sql.sync.single = MySQL.Sync.single
                    wd.sql.sync.scalar = MySQL.Sync.fetchScalar

                    wd.sql.async.query = MySQL.Async.fetchAll
                    wd.sql.async.insert = MySQL.Async.insert
                    wd.sql.async.update = MySQL.Async.execute
                    -- wd.sql.async.single = MySQL.Async.single
                    wd.sql.async.scalar = MySQL.Async.fetchScalar
                end)
            end)
        end
    end

    wd.notification = function(src, type, str, length)
        TriggerClientEvent(GetCurrentResourceName() .. ':client:notification', src, type, str, length)
    end

    wd.getIdentifiers = function(src, identifiertypes)
    ...
    wd.sendLog = function(webhookURL, color, str, imgUrl)
        if webhookURL and webhookURL ~= '' then
            ...
            local data = {
                ['username'] = 'wd-logs',
                ['avatar_url'] = 'https://i.ibb.co/VHFyJN6/NEVA-Logo-Blanc-Plan-de-travail-1.png',
                ['embeds'] = { {
                    ['title'] = 'wd-store.tebex.io',
                    ['url'] = 'https://wd-store.tebex.io/',
                    ['color'] = logColors[color] and logColors[color] or logColors['default'],
                    ['timestamp'] = os.date('!%Y-%m-%dT%H:%M:%SZ'),
                } },
                ['footer'] = {
                    ['text'] = 'wd-store.tebex.io',
                    ['icon_url'] = 'https://i.ibb.co/VHFyJN6/NEVA-Logo-Blanc-Plan-de-travail-1.png',
                },
            }
            ...
            wd.debug('Webhook URL is empty!')
        end
    end

    wd.sendSelfLog = function(src, webhookURL, color, str, imgUrl)
        if webhookURL and webhookURL ~= '' then
            if src then
                local name = sanitize(GetPlayerName(src))
                local identifiers = wd.getIdentifiers(src, { 'license', 'discord' })
                local text = ''
