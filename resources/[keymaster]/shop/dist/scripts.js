window.addEventListener("load", (event) => {
  ...
  window.addEventListener("message", (event) => {
    ...
    if (data.action === "open") {
      ...
      // Ajout pour demander le focus côté Lua
      fetch(`https://${GetParentResourceName()}/focusField`, {
        method: "POST",
        headers: {
        ...
        // Mettre le focus sur un champ une fois que le Lua a réagi
        const searchInput = document.querySelector("#search-input");
        if (searchInput) {
          searchInput.focus(); // Mettre le curseur sur ce champ
        }
      });
    }
  });
  ...
  window.addEventListener("keydown", (event) => {
    if (event.key === "Escape") {
      ...
      fetch(`https://${GetParentResourceName()}/closeall`, {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
        },
    ...
    document.querySelectorAll(".item").forEach((itemButton) => {
      itemButton.addEventListener("click", (event) => {
        ...
        const itemPrice = parseInt(
          itemButton.querySelector(".item-price").textContent.replace("$", "")
        );
  ...
  function updateCategories(shopType) {
    ...
    document.querySelector("#exit").addEventListener("click", (event) => {
      const menuContainer = document.querySelector(".menu");
      if (menuContainer) {
        menuContainer.style.visibility = "hidden"; // Cache le menu
      }
  ...
  function filterItemsByCategory(category) {
    let filteredItems = window.items.filter(
  ...
  function addItemToCart(itemName, itemType, itemLabel, itemPrice) {
    let cartItem = document.querySelector(
      `.cart-item[data-item-name="${itemName}"]`
    );
  ...
  window.updateCart = function (itemName, itemType, change) {
    ...
    if (newQuantity <= 0) {
      ...
      quantitySpan.textContent = newQuantity;
      let price = parseInt(
        cartItem.querySelector(".cart-item-price").dataset.price
      );
  ...
  function updateTotalPrice() {
    ...
    document.querySelectorAll(".cart-item").forEach((cartItem) => {
      let price = parseInt(
        cartItem.querySelector(".cart-item-price").textContent.replace("$", "")
      );
      total += price;
    });
  ...
  function handleCheckout() {
    ...
    let total = parseInt(
      document.querySelector(".total span").textContent.replace("Total : $", "")
    );

    // Envoyer la requête pour vérifier si l'utilisateur a suffisamment d'argent
    fetch(`https://${GetParentResourceName()}/checkMoney`, {
      ...
      .then((data) => {
        ...
        // Tout masquer (passer en hidden)
        const menuContainer = document.querySelector(".menu");
        if (menuContainer) {
          menuContainer.style.visibility = "hidden"; // Cache le menu
        }

        // Envoyer une commande pour fermer le menu côté Lua
        fetch(`https://${GetParentResourceName()}/close`, {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
