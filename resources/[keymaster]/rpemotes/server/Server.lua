...
local function addKeybindEventHandlers()
    ...
    RegisterNetEvent("rp:ServerKeybindGrab", function()
        ...
        MySQL.query('SELECT keybind1, emote1, keybind2, emote2, keybind3, emote3, keybind4, emote4, keybind5, emote5, keybind6, emote6 FROM `dpkeybinds` WHERE `id` = @id'
            ...
            if kb and kb[1].keybind1 ~= nil then
...
RegisterNetEvent('Wezor:sendAnimation', function(src, animName, animVariation)
    TriggerClientEvent('Wezor:PlayAnimation', src, animName, animVariation)
end)

RegisterNetEvent("Wezor:RequestToClient", function(src)
    if not src then return end
    TriggerClientEvent('Wezor:requestAnim', src, source)
end)

RegisterCommand("resetEmotesBind", function(source, args)
    print(source)
    local srcid = GetPlayerIdentifier(source)
    print(srcid)
    MySQL.update("UPDATE dpkeybinds SET emote1=@emote WHERE id=@id", { id = srcid, emote = "" })
    MySQL.update("UPDATE dpkeybinds SET emote2=@emote WHERE id=@id", { id = srcid, emote = "" })
    MySQL.update("UPDATE dpkeybinds SET emote3=@emote WHERE id=@id", { id = srcid, emote = "" })
    MySQL.update("UPDATE dpkeybinds SET emote4=@emote WHERE id=@id", { id = srcid, emote = "" })
    MySQL.update("UPDATE dpkeybinds SET emote5=@emote WHERE id=@id", { id = srcid, emote = "" })
    MySQL.update("UPDATE dpkeybinds SET emote6=@emote WHERE id=@id", { id = srcid, emote = "" })
    TriggerClientEvent("rp:ClientKeybindGet", srcid, "num4", "", "num5", "", "num6", "", "num7", "", "num8", "", "num8", "")
end)