...
RegisterNetEvent('fl_core:getAnim', function(cb)
    cb(ChosenAnimation, ChosenDict, CurrentAnimationName, CurrentTextureVariation)
end)

RegisterNetEvent('Wezor:requestAnim', function(src)
    TriggerServerEvent('Wezor:sendAnimation', src, CurrentAnimationName, CurrentTextureVariation)
end)

RegisterNetEvent('Wezor:PlayAnimation', function(anim, variation)
...
RegisterCommand('e', function(source, args, raw)
    if args[1] == "hump" then
        TriggerEvent("FlashLand:displayNotification", "~r~Script ANTI-BICHOU By Goldenn")
        return
    end
...
if Config.MenuKeybindEnabled then
    RegisterCommand('emoteui', function() FlashLandEmotesMenu() end, false)
    RegisterKeyMapping("emoteui", "Open rpemotes menu", "keyboard", Config.MenuKeybind)
else
    RegisterCommand('emotemenu', function() FlashLandEmotesMenu() end, false)
end
...
function CheckForVoidAndTeleport(playerPed)
    -- Obtenir la position actuelle du joueur
    local playerPos = GetEntityCoords(playerPed)
        
    -- Calculer la position devant le joueur
    local forwardPos = GetOffsetFromEntityInWorldCoords(playerPed, 0.0, 1.5, 0.0)  -- 1.5 unités devant le joueur

    -- Obtenir la coordonnée Z du sol à la position devant le joueur
    local _, groundZ = GetGroundZFor_3dCoord(forwardPos.x, forwardPos.y, forwardPos.z, false)

    -- Si la coordonnée Z est très éloignée, cela indique probablement du vide
    if math.abs(forwardPos.z - groundZ) > 1 then  -- Ajuster cette valeur selon les besoins
        -- Calculer la position juste derrière le joueur
        local behindPos = GetOffsetFromEntityInWorldCoords(playerPed, 0.0, -1.0, 0.0)  -- 1.5 unités derrière le joueur

        -- Téléporter le joueur derrière lui
        SetEntityCoords(playerPed, behindPos.x, behindPos.y, behindPos.z)
    end
end
...
function EmoteCancel(force)
    ...
    AnimationThreadStatus = false
    CheckStatus = false
    TriggerEvent('fl_core:setPlayerAtLastPosition', function(position)
        if position ~= nil then
            SetEntityCoords(PlayerPedId(), position.x, position.y, position.z)
            TriggerEvent('fl_core:setPlayerResetPosition')
        end
    end)
end
