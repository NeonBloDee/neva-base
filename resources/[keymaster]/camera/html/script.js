...
function playerDetected(e) {
  ...
  $(".info-box .header").text("Informations suspect");

  $(".info-box .img-box").empty();
  $(".info-box .img-box").html(
    `<img src="../html/images/user-face.png" alt="avatar" class="face">`
  );

  if (e.signal && e.weaponDetected) {
    $(".signal").text("Très dangereux");
    $(".signal").css("background-color", "rgba(255, 0, 0, 0.35)");
    $(".signal").css("color", "#FF0000");
  } else if (e.weaponDetected || e.maskDetected) {
    $(".signal").text("Dangereux");
  ...
  if (e.maskDetected) {
    const item = `
            <div class="dedected-item">
                <div class="dedected-name">Masque détecté</div>
  ...
  if (e.weaponDetected) {
    const item = `
            <div class="dedected-item">
                <div class="dedected-name">Arme détectée</div>
                <div class="warning-icon active" style="left: 57%;"> 
                  <svg width="20" height="21" viewBox="0 0 20 21" fill="none" xmlns="http://www.w3.org/2000/svg">
...
function playerNotDetected(e) {
  $(".info-box .header").text("Aucun suspect détecté");
  $(".info-box .name").text("");
  $(".dedected-list").empty();
  $(".info-box .img-box").empty();
  $(".signal").text("");
...
function sortCamList(camData) {
  return camData.sort((a, b) => {
    if (sortBy === "risk") {
      const riskLevel = (cam) => {
        if (!cam.broken && cam.hasWeapon && cam.hasMask && cam.grouping)
          return 3;
        if (!cam.broken && (cam.hasWeapon || cam.hasMask || cam.grouping))
          return 2;
        return 1;
...
function openCamList(e) {
  ...
  let camData = camsData.map((cam, index) => {
    const detectedData =
      e.camDetectionData && e.camDetectionData[index]
        ? e.camDetectionData[index].detected
        : [];
    const grouping =
      e.camDetectionData && e.camDetectionData[index]
        ? e.camDetectionData[index].grouping
        : false;
    ...
    let allBg =
      "linear-gradient(331deg, rgba(0, 0, 0, 0.00) 8.45%, rgba(82, 255, 93, 0.20) 100%), rgba(255, 255, 255, 0.03) !important;";
    let nameColor = "white";
    ...
    if (!cam.broken && (hasWeapon || hasMask || grouping)) {
      ...
      allBg =
        "linear-gradient(331deg, rgba(0, 0, 0, 0.00) 8.45%, rgba(255, 149, 0, 0.20) 100%), rgba(255, 255, 255, 0.03) !important;";
      nameColor = "white";
    ...
    if (cam.broken) {
      ...
      allBg =
        "linear-gradient(331deg, rgba(0, 0, 0, 0.00) 8.45%, rgba(207, 207, 207, 0.20) 100%), rgba(255, 255, 255, 0.03) !important";
      warningContent = ``;
      warningText = "Aucun signal";
      ...
      codeBg =
        "radial-gradient(94.82% 77.03% at 50% 50%, rgba(255, 255, 255, 0.08) 0%, rgba(153, 153, 153, 0.00) 100%);";
    ...
    if (!cam.broken && hasWeapon & hasMask & grouping) {
      ...
      warningContent = `
                <div class="error-img" style="background: url(../html/images/${camImage}) lightgray 50% / cover no-repeat;">
                    ...
                    <div class="hr"></div>
                    <div class="text">Alerte !</div>
                    <div class="text-2">Alerte !</div>
                </div>`;
      warningText = "Risque élevé";
      warningColor = "#FF0000";
      warningBg = "rgba(115, 23, 23, 0.24) !important;";
      allBg =
        "linear-gradient(331deg, rgba(0, 0, 0, 0.00) 8.45%, rgba(232, 45, 45, 0.20) 100%), rgba(255, 255, 255, 0.03) !important;";
      nameColor = "white";
      gpsImg = "gps-icon-white";
      indexColor = "white";
      codeBg = "rgba(255, 255, 255, 0.08);";
  ...
  camData.forEach((cam, index) => {
    ...
    if (existingItem.length > 0) {
      ...
      existingItem
        .find(".warning")
        .text(cam.warningText)
        .css({ color: cam.warningColor, background: cam.warningBg });
...
$(document).on("click", ".close-icon , .alt-icon", function (e) {
  closeCam();
  $.post("https://camera/closeNui", JSON.stringify({}));
});
...
document.addEventListener("keydown", function (e) {
  if (e.key == "Control") {
    if (currentCam == "cam") {
      $.post("https://camera/closeFocus", JSON.stringify({}));
    }
...
$(document).ready(function () {
  ...
  setInterval(function () {
    ...
    $(".record-box").css(
      "box-shadow",
      `0px 0px 0px 5px rgba(153, 0, 0, ${opacity})`
    );
  }, 100);
});
