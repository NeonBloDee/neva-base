...
Citizen.CreateThread(function()
    while true do
        ...
        if NetworkIsPlayerActive(PlayerId()) then 
            ...
            TriggerServerEvent('camera:initializeCameras')
            break
        end 
    end
end)

RegisterNetEvent('camera:createCam')
AddEventHandler('camera:createCam', function(serverCameras)
...
RegisterNetEvent('camera:updateCamera')
AddEventHandler('camera:updateCamera', function(index, camData)
...
RegisterNetEvent('camera:signalcutter')
AddEventHandler('camera:signalcutter', function()
    ...
    for k, camData in pairs(createdCameras) do
        ...
        if #(playerCoords - camCoords) < 5.0 and not camData.broken then 
            ...
            FRKN.MinigameConfig["stickGame"](function(result)
                if result then
                    TriggerServerEvent('camera:breakCamera', k)
...
local showInstruction = true -- Make it global to this file
    ...
    while true do
        ...
        if distance <= 3 then
            ...
            if showInstruction then
                DrawInstructionBarNotification(camCoords.x, camCoords.y, camCoords.z, "Appuyez sur [ ~r~E~w~ ] pour intéragir")
                ...
                if IsControlJustPressed(0, 38) then
                    ...
                    if canAccess and not IsPedInAnyVehicle(playerPed, true) then
                        showInstruction = false
                        TriggerEvent("camera:camList")
                    end
                end
            end
        else
    ...
    while true do
        ...
        if distance < 50.0 then
            ...
            if isWearingMask and isArmed and distance < 10.0 and not alarmActive then
                alarmActive = true
                TriggerServerEvent("camera:triggerAlarm", camCoords)
...
RegisterNetEvent('camera:camList')
AddEventHandler('camera:camList', function()
...
RegisterNetEvent("camera:playAlarm")
AddEventHandler("camera:playAlarm", function(coords)
