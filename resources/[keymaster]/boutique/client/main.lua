...
RegisterNUICallback("boutique:winCase", function(data, cb)
    ...
    if data.sell then 
        ...
        if data.gain.type == "Vehicle" then 
            TriggerServerEvent("Boutique:Rewards", "vehicle", {model = data.gain.item, coins = 0, caseCoins = data.caseCoins, unique = data.gain.unique, lootbox = true})
        elseif data.gain.type == "Weapon" then
            TriggerServerEvent("Boutique:Rewards", "weapon", {name = data.gain.item, coins = 0, unique = data.gain.unique, lootbox = true})
        elseif data.gain.type == "item" then 
            -- print('ici Client')
            TriggerServerEvent("Boutique:Rewards", "item", {name = data.gain.item, coins = 0, unique = data.gain.unique, lootbox = true})
        elseif data.gain.type == "Money" then 
            TriggerServerEvent("Boutique:Rewards", "money", {count = data.gain.item, coins = 0, unique = data.gain.unique, lootbox = true})
        elseif data.gain.type == "Coin" then 
            TriggerServerEvent("Boutique:Rewards", "coins", {count = data.gain.item, coins = 0, unique = data.gain.unique, lootbox = true})
        elseif data.gain.type == "Item" then 
            TriggerServerEvent("Boutique:Rewards", "item", {name = data.gain.item, count = data.gain.count, coins = 0, unique = data.gain.unique, lootbox = true})
        end
    end
