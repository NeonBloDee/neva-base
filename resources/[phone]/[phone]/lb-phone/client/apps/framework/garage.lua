...
RegisterNUICallback("Garage", function(data, cb)
    ...
    if action == "getVehicles" then
        lib.TriggerCallback("sunny:garage:myvehicle", function(vehicles)
            if vehicles and #vehicles > 0 then
                for i = 1, #vehicles do
                    vehicles[i].model = GetVehicleLabel(vehicles[i].model)
                    -- Si vous utilisez un système de verrouillage, vous pouvez définir l'état de verrouillage ici
