  ...
  shouldBuild(resourceName) {
    try {
      ...
      try {
        ...
        const r = GetResourcePath(resourceName);
        const f = fs.readdirSync(r);
  ...
  build(resourceName, cb) {
    (async () => {
      while (buildingInProgress && currentBuildingModule !== resourceName) {
        console.log(
          `yarn is currently busy: we are waiting to compile ${resourceName}`
        );
        await sleep(3000);
      }
      ...
      const proc = child_process.fork(
        ...
        [
          ...
          "--ignore-scripts",
          "--cache-folder",
          path.join(initCwd, "cache", "yarn-cache"),
          "--mutex",
          "file:" + path.join(initCwd, "cache", "yarn-mutex"),
        ],
        {
