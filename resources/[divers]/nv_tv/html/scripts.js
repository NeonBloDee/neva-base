...
var televisionresourcename = "nv_tv";
...
window.addEventListener("message", function (event) {
  ...
  if (item.message == "tvshow") {
    ...
    document.getElementsByClassName("currentlytexttype")[0].innerHTML =
      item.tvurltypedata;
    volumeslider.value = item.tvvolumedata;
    if (item.tvurldata == "") {
      document.getElementsByClassName("currentlytextdata2")[0].innerHTML = "";
      document.getElementsByClassName("currentlytext")[0].innerHTML =
        "Aucune vidéo en lecture";
    } else {
      document.getElementsByClassName("currentlytext")[0].innerHTML =
        "En cours de lecture";
      document.getElementsByClassName("currentlytextdata2")[0].innerHTML =
  ...
  if (item.message == "changemode") {
    // Traduire les modes en français
    let modeText = item.modedata;
    if (modeText === "MOVE") modeText = "DÉPLACER";
    if (modeText === "ROTATE") modeText = "ROTATION";
    document.getElementById("currentmodedata").innerHTML = modeText;
  }
  ...
  document.onkeyup = function (data) {
    if (open) {
      if (data.which == 27) {
        $.post(
          "https://" + televisionresourcename + "/quit",
          JSON.stringify({})
        );
      }
    }
...
$(".buttonplay").click(function () {
  $.post(
    "https://" + televisionresourcename + "/playvideo",
    JSON.stringify({
...
function volumechange(e) {
  $.post(
    "https://" + televisionresourcename + "/volumechange",
    JSON.stringify({
      volumedata: e.value,
    })
  );
