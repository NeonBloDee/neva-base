...
if Config.Framework == "custom" then
	--TriggerClientEvent("nv_tv:TVRemote", playersource) remote control trigger you can implement this event to your usable item for example
	
	--TriggerClientEvent("nv_tv:InstallVehicleTVClient", playersource) Trigger for install vehicle to tv

	RTXTV.RegisterServerCallback("nv_tv:CheckVehicleTV", function(source, cb, tvplatedata)
	...
	RegisterServerEvent("nv_tv:InstallVehicleTV")
	AddEventHandler("nv_tv:InstallVehicleTV", function(tvplatedata, vehiclenetworkdata)
		local playersource = source
		if tvplatedata ~= nil then	
			MySQL.Async.fetchScalar('SELECT 1 FROM playervehiclestable WHERE plate = @plate', {
				...
				if found then
					...
					TriggerClientEvent("nv_tv:Notify", playersource, Language[Config.Language]["tvinstalled"])
					TriggerClientEvent("nv_tv:TVInstalledVehicleClient", playersource, vehiclenetworkdata)
				else								
					TriggerClientEvent("nv_tv:Notify", playersource, Language[Config.Language]["vehiclenotowned"])
				end
			end)			
		end
	end)

	RegisterServerEvent("nv_tv:InstallVehicleTVCustom")
	AddEventHandler("nv_tv:InstallVehicleTVCustom", function(tvplatedata, vehiclenetworkdata)
		local playersource = source
		if tvplatedata ~= nil then	
			MySQL.Async.fetchScalar('SELECT 1 FROM playervehiclestable WHERE plate = @plate', {
				['@plate'] = tostring(tvplatedata),
