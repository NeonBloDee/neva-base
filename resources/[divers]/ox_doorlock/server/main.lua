...
local function setDoorState(id, state, lockpick)
	...
	if door then
		...
		if source then
			local message = state == 0 and "Impossible de déverrouiller la porte" or "Impossible de verrouiller la porte"
			TriggerClientEvent('esx:showNotification', source, message)
			--lib.notify(source, { type = 'error', icon = 'lock', description = state == 0 and 'cannot_unlock' or 'cannot_lock', position = 'bottom'})
		end
	end
...
RegisterCommand('doorlock', function(source)
	if ESX.GetPlayerFromId(source).group == 'fondateur' then
